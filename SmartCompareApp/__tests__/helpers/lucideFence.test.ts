...
describe('lucideFence helper — lucideRefs() reach (W3-9)', () => {
  ...
  // R1 widened (fixer, adversary finding 3): Metro 0.83.3 reads a require's
  // module name with babel `evaluate()` (collectDependencies.js
  // `getModuleNameFromCallArgs`), so a template literal or a `const` binding
  // registers the barrel exactly like a string literal does. Mutation that
  // must redden it: read only a `StringLiteral` argument.
  it('R1: reports a template-literal or const-bound require the way Metro resolves it', () => {
    expect(lucideRefs('x.js', 'const L = require(`' + LUCIDE + '`);')).toEqual([
      { file: 'x.js', shape: 'require', exported: LUCIDE, local: LUCIDE },
    ]);
    expect(
      lucideRefs('x.js', `const name = '${LUCIDE}';\nconst L = require(name);`)
    ).toEqual([{ file: 'x.js', shape: 'require', exported: LUCIDE, local: LUCIDE }]);
  });
  ...
  // R2, the other node type (fixer): under `createImportExpressions: true`
  // (the Babel 8 default) the same code parses as an `ImportExpression`, so
  // the helper's second dynamic-import branch is reachable and pinned here,
  // not merely present. Mutation that must redden it: delete the
  // ImportExpression visitor.
  it("R2: reports the `ImportExpression` form (createImportExpressions) as shape 'dynamic-import'", () => {
    expect(
      lucideRefs('x.ts', `export async function f() { return import('${LUCIDE}'); }`, {
        createImportExpressions: true,
      })
    ).toEqual([{ file: 'x.ts', shape: 'dynamic-import', exported: LUCIDE, local: LUCIDE }]);
  });
  ...
  it("R3: reports `import L = require('lucide-react-native')` as shape 'require'", () => {
    ...
    expect(refs).toHaveLength(1);
    expect(refs[0]).toMatchObject({ file: 'x.ts', shape: 'require', exported: LUCIDE, local: 'L' });
  });
  ...
  describe('R5 (pin): the pre-existing import shapes classify exactly as before', () => {
    ...
    // A NAMED re-export is an `ExportNamedDeclaration`, a different visitor
    // from `export *` (`ExportAllDeclaration`). The B1 plugin visits
    // `ImportDeclaration` only, so this form keeps the whole barrel under
    // Metro. Mutation that must redden it: delete the ExportNamedDeclaration
    // visitor (it survived every other case — adversary finding 1).
    it("`export { Camera } from` → re-export", () => {
      expect(lucideRefs('x.ts', `export { Camera } from '${LUCIDE}';`)).toEqual([
        { file: 'x.ts', shape: 're-export', exported: LUCIDE, local: LUCIDE },
      ]);
    });
