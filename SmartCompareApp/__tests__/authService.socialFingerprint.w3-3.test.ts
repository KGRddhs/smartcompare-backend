 ...
 * C6 (google), C6-apple (cancel + no-identity-token), and the register-header pin (§6 — no existing client test asserts it;
...
describe('W3-3 pins — the fingerprint must not change anything else', () => {
  ...
  it('C6-apple (pin): a cancelled apple sign-in never reads the fingerprint', async () => {
    // Same placement rule as C6 for the Apple path: the read sits AFTER
    // apple.signInAsync() has returned an identity token. (Apple's nonce is
    // generated with expo-crypto BEFORE the sheet opens — that is unchanged and
    // not what this pins; this pins that the FINGERPRINT is never read.)
    // Mutation that reddens this: move the fingerprint read above
    // apple.signInAsync(). Added in the fix round — the adversary showed that
    // pure move survived the Google-only C6.
    const cancelled: any = new Error('cancelled');
    cancelled.code = 'ERR_REQUEST_CANCELED';
    mockAppleSignInAsync.mockRejectedValue(cancelled);
  ...
  it('C6-apple (pin): an apple credential without an identity token never reads the fingerprint', async () => {
    // The no-idToken early return is the other pre-fetch exit on the Apple
    // path; same mutation reddens it.
    mockAppleSignInAsync.mockResolvedValue({ identityToken: null });

    const result = await signInWithApple();

    expect(result).toEqual({ success: false, error: 'Failed to get Apple identity token' });
    expect(mockGetDeviceFingerprint).not.toHaveBeenCalled();
    expect(mockFetch).not.toHaveBeenCalled();
  });
