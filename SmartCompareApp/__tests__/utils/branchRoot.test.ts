...
describe('backOrExit', () => {
  // The cold-start half must exit to the MOUNTED branch root on every branch —
  // a backOrExit that hard-coded one root (e.g. reset -> 'Auth') would leave
  // the back arrow dead for authed / needs-preferences invitees.
  it.each(BRANCHES)(
    '%s branch, nothing beneath us: never goBack, resets onto <root> the router accepts',
    (_label, routeNames, expectedRoot) => {
      const navigation: any = {
        getState: () => ({ routeNames: [...routeNames] }),
        reset: jest.fn(),
      ...
      const payload = navigation.reset.mock.calls[0][0];
      expect(payload.routes).toHaveLength(1);
      expect(payload.routes[0].name).toBe(expectedRoot);
      expect(routerOutcome(payload, routeNames)).not.toBeNull();
    },
  ...
  it.each(BRANCHES)(
    '%s branch, something beneath us: goBack once, never resets',
    (_label, routeNames) => {
      const navigation: any = {
        getState: () => ({ routeNames: [...routeNames] }),
        reset: jest.fn(),
