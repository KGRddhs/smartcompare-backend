...
jest.mock('@react-navigation/native', () => ({
  ...
  // W3-15's linking.ts creates the container ref at module scope; the real
  // package loads under jest.config.js's @react-navigation transform (W3-15).
...
describe('a reset-password link that carries no usable token still reaches the screen', () => {
  ...
  it.each([
    ['reset-passwordx#access_token=tok-X&type=recovery'],
    ['/reset-passwords?access_token=tok-X&type=recovery'],
    ['reset-password-old'],
  ])('%s is NOT the reset route — the override matches the whole first segment only', (url) => {
    const state = resolve(url);
    expect(focused(state)).not.toEqual(['Auth', 'ResetPassword']);
    expect(consumePendingRecovery()).toBeNull();
  });
