...
describe('W3-4 — the back arrow on a cold-start deep link', () => {
  ...
  // Every branch, not only unauth: a back arrow whose exit ignored the mounted
  // set (e.g. a hard-coded reset -> 'Auth') would pass an unauth-only cell and
  // be dead again for an authed / needs-preferences invitee. Literal roots
  // (RULING 4), router-verified.
  it.each(BRANCHES)(
    '[%s] ReferralLanding: nothing beneath us -> exits to the branch root, never goBack',
    async (_branch, routeNames, expectedRoot) => {
      const nav = makeNav(routeNames, false);
      fireEvent.press(await renderLandingBack(nav));

      expect(nav.goBack).not.toHaveBeenCalled();
      await waitFor(() => expect(nav.reset).toHaveBeenCalledTimes(1));
      const payload = nav.reset.mock.calls[0][0];
      expect(routerOutcome(payload, 'ReferralLanding', routeNames)).not.toBeNull();
      expect(payload.routes[0].name).toBe(expectedRoot);
    },
    20000
  );
  ...
  it.each(BRANCHES)(
    '[%s] InviteeQuiz step 0: nothing beneath us -> exits to the branch root, never goBack',
    async (_branch, routeNames, expectedRoot) => {
      const nav = makeNav(routeNames, false);
      ...
      await waitFor(() => expect(nav.reset).toHaveBeenCalledTimes(1));
      const payload = nav.reset.mock.calls[0][0];
      expect(routerOutcome(payload, 'InviteeQuiz', routeNames)).not.toBeNull();
      expect(payload.routes[0].name).toBe(expectedRoot);
    },
    20000
  );
