...
// A3 mutex follow-up: the boot refresh now goes through api.ts's module-scope
// singleton Promise, so this suite has to exercise the REAL api.ts — mocking
// the api MODULE would mock away the very dedup the boot path depends on.
// The transport is mocked one level lower instead (axios), which keeps every
// `mockPost` assertion below identical.
...
beforeEach(async () => {
  ...
  __resetSessionListeners();
  // Several cases below deliberately leave a refresh POST in flight forever;
  // without this the next case would coalesce onto that dead Promise.
...
/**
 * A3 follow-up — the boot refresh must not spend the single-use refresh
 * token a second time.
 *
 * Supabase rotates the refresh token on every successful POST /auth/refresh,
 * so it is SINGLE-USE, and the backend says so in its own docstring
 * (app/api/auth_routes.py::refresh): "If two concurrent clients race to
 * refresh with the same token, only one wins; the loser gets a 401 ...
 * Deduping is therefore a CLIENT-SIDE responsibility — the mobile app must
 * hold a module-scope singleton Promise around the refresh call."
 *
 * That singleton is api.ts's getOrStartRefresh(). Moving the boot refresh OFF
 * the render path put it in genuine flight alongside the first authed calls of
 * a launch (App.tsx's push-token PUT, Home's referral-status GET on focus),
 * every one of which rides the SAME cached-and-expired Bearer and enters the
 * 401 interceptor's refresh. A boot refresh that called refreshSession()
 * directly would therefore issue a SECOND concurrent refresh with the same
 * token: the loser's 401 lands on refreshSession's clearSession() +
 * sessionInvalid, which routes the user back to the Auth stack at launch and
 * deletes the winner's freshly stored tokens.
 */
describe('boot refresh shares the refresh mutex (A3 / R9)', () => {
  const FRESH_USER = {
    id: 'u-cached',
    email: 'cached@qaren.app',
    display_name: 'Renamed Elsewhere',
  ...
  it('a 401 landing DURING boot joins the boot refresh: ONE POST, one token spend', async () => {
    ...
    // The 401 interceptor's refresh entry point, fired while the boot refresh
    // is still in flight.
    const interceptorRefresh = apiModule.__testRefreshDedup();
    await settleBackground();

    // Bypassing the mutex would show up here as a SECOND POST carrying the
    // SAME refresh token.
    expect(mockPost).toHaveBeenCalledTimes(1);
    expect(mockPost).toHaveBeenCalledWith(
      '/api/v1/auth/refresh',
  ...
  it('a boot starting while a 401 refresh is in flight joins it, and still re-syncs the user', async () => {
    ...
    // The interceptor wins the start this time (its refresh carries no
    // per-call deadline — the mid-session budget).
    const interceptorRefresh = apiModule.__testRefreshDedup();
    await settleBackground();
    expect(mockPost).toHaveBeenCalledTimes(1);
    expect(mockPost).toHaveBeenCalledWith(
    ...
    expect(mockPost).toHaveBeenCalledTimes(1);
    // A coalesced boot has no result object of its own, so it re-reads what
    // the winner persisted — the UI still learns about the fresher user.
    expect(onSessionRefreshed).toHaveBeenCalledTimes(1);
