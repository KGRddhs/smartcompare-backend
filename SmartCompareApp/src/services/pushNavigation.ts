 ...
 * prefix match is done here. The comparison is case-insensitive (spec P9);
...
export function pathFromPushUrl(
  ...
  for (const prefix of prefixes) {
    ...
    if (!prefix.endsWith('/') && rest !== '' && !/^[/?#]/.test(rest)) continue;
    return rest.replace(/^\/+/, '');
  }
  return undefined;
}
...
export function handlePushResponse(response: unknown): boolean {
  ...
  // (Notifications.types.d.ts:390-392). The ONE `typeof url === 'string'`
  // guard lives in pathFromPushUrl (tsc requires it there, since the url is
  // `unknown`); a second copy here would be unpinnable duplication — P4b
  // reddens if that guard is removed.
