...
export async function signInWithGoogle(): Promise<AuthResponse> {
  try {
    ...
    // sheet has returned an id token, so a cancelled sign-in never calls
    // getDeviceFingerprint (pinned, C6); swallowed exactly like register, so sign-in never
...
export async function signInWithApple(): Promise<AuthResponse> {
  try {
    ...
    // the anti-farming controls that read it fail OPEN. Read AFTER the native
    // sheet has returned an id token, so a cancelled sign-in never calls
    // getDeviceFingerprint (pinned, C6-apple; the nonce above still uses crypto,
    // that is unchanged); swallowed exactly like register, so sign-in never
    // blocks on a fingerprint failure.
    let fingerprint: string | null = null;
    try {
      fingerprint = await getDeviceFingerprint();
