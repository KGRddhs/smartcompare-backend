...
**Active runtime:** Bundle D TestFlight Readiness + Path A R1/R2 merged 2026-05-25 (`6ee3aa5`). **Bundle E S2 merged 2026-05-30** (`d2d9386` — onboarding 17-step rewrites + LoadingScreenVariants + Profile/EditPrefs OptionRow + SlideTransition wrap). **Bundle E S3 merged 2026-05-30** (`443417c` — 4-Opus team with ring cross-QA closed the visual fidelity arc: HomeScreen/Profile/EditProfile/Results/History REWRITEs per Claude-Design JSX + product image pipeline plumbed end-to-end). Latest `eas update` group `48d083b6-aceb-47e3-a46c-8260c06ab538` on `preview`. **AWAITS Ahmed device walk (tomorrow 2026-05-31) across 6 surfaces — that's the TestFlight ship gate.** `STREAM_HARD_CAP_SECONDS=25.0`. `SCRAPING_MODE=soft` URL gate wired.
