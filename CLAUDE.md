...
**Active runtime:** **Bundle B Session 1 "Foundation" shipped 2026-06-10 (main `ea4be1b` + close-out through `771fc61`).** B.0 Bahrain registry wired into Tier 1.5 (registry 30→37 sources; hit-rate counters in `/admin/costs`); migrations 027–032 LIVE (B.1 observability schema); eval backbone operational — **S1 baseline = 21.0% weighted, eval_runs row `4aee8e88-da97-41b3-974b-3e75c2c9c10e`** (regression-gate anchor; runbook `docs/runbooks/qaren-eval.md`, measurement runs ALWAYS `--concurrency 1`); gold set 200 ratified (`docs/runbooks/qaren-gold-set.md`); analytics funnels un-darkened (events allowlist superset test); Google sign-in nonce-echo removed (415dafb). Sentry 1 unresolved (PYTHON-FASTAPI-9 defer-and-monitor; N/M same-day-resolved = key depletion). Latest `eas update` group `ba52fdf9-e5c1-41cd-9bd4-cb5a71c183d7` on `preview`. `SERPER_API_KEY` rotated to `3d304e...` 2026-06-10 (mid-baseline depletion; keep local `.env` in sync on every rotation — it was found 2 rotations stale). **S2 "Intelligence" opens with `docs/plans/2026-06-10-bundle-b-s2-prep-notes.md`** (2 Ahmed UX decisions pending there). Full narrative: `docs/SESSION_BUNDLES.md` Session 60. **Pre-S1:** Sprint A + B0 Hardening shipped 2026-06-09 (`3f4f8d1`) — phantom-tie 91%→0%, `STREAM_HARD_CAP_SECONDS=30.0`, `ENABLE_FIRECRAWL/SCRAPEDO/PAGE_SCRAPE/DEBUG_STAGE_TIMINGS=true`.
