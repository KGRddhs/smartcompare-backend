...
**M20 W3 flag (#110 — winner-prose reconciliation, `response_builder.py` chokepoint + the SSE `verdict` emit in `structured_comparison_service.py`, 2026-09-01).**
- `ENABLE_WINNER_PROSE_RECONCILE` (**default OFF**) — on a `WINNER_INDEX_MISMATCH` (deterministic scoring winner != GPT's `comparison["winner_index"]`). **Effect ON:** `overview.winner.declaration` / `.reason` / `.key_tradeoff` (and their fan-outs: `result["recommendation"]`, the BC `comparison` alias) are REPLACED with the deterministic template strings for the shipped winner via the shared `response_builder.deterministic_verdict_fields` helper (also the string source for `_deterministic_partial_verdict`, so the two template sites cannot drift), and the SSE `verdict` event carries the deterministic index + the same reconciled prose, so the streamed winner never flips between the `verdict` and `complete` events. **Flag OFF:** byte-identical to the merged #99 base — name/declaration reconciled UNFLAGGED, GPT reason/key_tradeoff survive unless they literally name the loser, SSE `verdict` still ships GPT's index (the mid-stream flip stands). Read PER CALL via `os.environ` (`response_builder._winner_prose_reconcile_enabled`). Indices-agree path and scoring-winner-None path are untouched at both flag states. The `WINNER_INDEX_MISMATCH` warning fires regardless of the flag (it is the only mismatch-rate telemetry). **Canary alone in its own window** — it changes user-facing verdict copy on the core surface. Note: flag ON also sidesteps the #123 prefix over-scrub on the mismatch path (template overwrite, no name-containment test); flag OFF #123 stands.
