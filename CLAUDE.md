...
**Active runtime (genuine-price CORRECTNESS — 🚨 needs a STRUCTURAL matcher REDESIGN in a FRESH session, PR #9 DO-NOT-MERGE):** After the external review FAIL + a 3rd dispatcher fix pass (which greened the 15 reproduced counterexamples + comm + a clean over-rejection sweep), a **COVERAGE-DRIVEN review workflow** (`.qa-correctness/review_coverage.mjs`; 7 agents, real-product enumeration, both directions, runtime-reproduced) found **42 verified findings (10 CRIT) the 2 prior HYPOTHESIS-driven review workflows MISSED** — the root cause is structural (`_selection_match` is SUBSET-based → candidate-ADDS-distinctive-token leaks across EVERY category: Canon R6→R6 Mark II, AirPods Pro→Pro 2, Magnesium→Glycinate, Samba OG→Classic, Aventus→Aventus Cologne; + missing axes lb/oz/%/shoe-size/pack/single-digit-model/shade/supplement-form; + cache reads unrevalidated; + KPI dead-code/too-strict). **USER DECISION: do the FULL category-canonical-identity redesign in a FRESH session** (this session's context is bloated). PLAN (committed): `docs/plans/2026-06-28-genuine-price-correctness-STRUCTURAL-FIXES.md` (clusters A keystone superset-guard / B numeric-axes / C form / D cache-read / E KPI-wiring / F chokepoint+category-inference / G over-rejection / H flag-off+telemetry); raw repros `.qa-correctness/coverage_findings.txt`; ready-to-paste kickoff + the lesson in the handoff. **LESSON (sharpened, [[feedback-coverage-driven-review]]): an adversarial review WORKFLOW is only as good as its COVERAGE — a hypothesis-driven review CONFIRMS the prompter and inherits the blind spots; a COVERAGE sweep (enumerate the full space from domain knowledge, both directions) is what falsifies. _(history below: the over-claimed pass + the external-review findings that started this.)_** The first FIX pass (`45f682b`) was declared "all 8 fixed" + comm-green + 2 review workflows — **but an independent external review REPRODUCED runtime leaks the in-session gate AND both workflows MISSED** (they only probed the cases the dispatcher PROMPTED — fragrances/accessory/gender — never sweeping electronics model-tokens, fashion, the cache-guard semantics, the KPI aggregate path, or AggregateOffer-lowPrice-only). **LESSON REPEATS (3rd time): a green gate + self-review + even adversarial workflows are NOT proof — an adversarial sweep is only as good as its COVERAGE; a workflow probing the prompter's hypotheses CONFIRMS them, it does not falsify the unconsidered cases.** Verified-reproduced still-broken (gate-ON): matcher still subset-based + DROPS SHORT model tokens → MacBook M2→M3 / Canon R6→R5 / Vitamin C→D / S24 256GB→storage-omitted / skincare 473ml→omitted / fashion White→Black / fashion M→L all ACCEPTED; `should_cache_price` True for missing-identity/missing-URL/OOS + cache key still REQUEST-derived (`scs:~4125`) + reads unrevalidated; `count_usable_exact_genuine` still circular (no truth_entry → Libre counts exact for Black Opium `(2,2)`; truth loader never wired into a run-mode); AggregateOffer lowPrice-ONLY accepted + missing-availability→`in_stock=True`; chokepoint allows missing identity+URL (the 16-test "calibration" left NO fail-closed URL/stock layer); JSON-LD literal brand-substring filter rejects a valid alias (YSL vs "Yves Saint Laurent") BEFORE the alias matcher; category inference None for fashion/grocery/makeup/skincare/haircare on HTML; flag-OFF byte-identity unproven (helper tests, not base-vs-branch golden byte-compare). **FIX ORDER (reviewer): failing E2E tests for EVERY counterexample FIRST → replace subset matching with category-DEFINED canonical identity axes (preserve M2/R6/C/D; enforce EVERY query-stated axis or pend) → cache key from RESOLVED identity + reject writes without exact-identity/valid-PDP/confirmed-stock + revalidate/bust reads → AggregateOffer needs a proven exact offer + unknown stock stays None → wire the truth corpus into a real eval cmd → golden base-vs-branch byte-compare for flag-OFF → THEN smoke20 + paid COLD/WARMED KPI.** Handoff: `memory/project_genuine_price_correctness_plan.md`. **(HISTORY — the first pass's over-claim, every line below now KNOWN-INCOMPLETE per the review above:) HOW each was [partially] fixed (all reproduced through the RUNTIME path FIRST — 16 RED→green):** B1 iHerb best-overlap→`_selection_match` gate + keep-title, pharmacy threads `query_name`+title; B2 the matcher `select_best` ACTUALLY runs (`_selection_match`) hardened — form-axis (deodorant/candle/lotion/**gift-set**), fragrance flanker near-equality, candidate-omits-query-axis fail-closed, **flagship-concentration** (Le Parfum/Parfum/Extrait), **gender CONTRADICTION** axis, **supplement-type** (isolate/k2) + `_BRAND_ALIAS_GROUPS` (YSL↔Yves Saint Laurent) — WITHOUT over-rejecting descriptive titles / one-sided "Pour Homme" / oz≡ml / EDT≡"Eau de Toilette" / diacritics; B3 `usable_exact_genuine_for_product(body,idx,truth_entry)` LOADS the truth + `is_exact_match` vs `truth.query` + confirmed-in-stock + valid-PDP-URL; B4 reject AggregateOffer RANGE on the exactness path (I5.8 single preserved) + `priceValidUntil` 365-day grace; B5 `select_best` require title+valid-PDP-URL — at SELECTION+cache+KPI **NOT the display chokepoint** (that over-rejected 16 established "genuine source_method+amount=showable" tests; B5 = "before selection OR cache write"); B6 `should_cache_price` (reuses `_selection_match`→never blocks a selected price) gates EVERY genuine write via `scs._persist_genuine_price`; B7 `guard_rejected`→`metadata.guard_rejected` + `public_price_view` (overview+legacy alias+SSE, flag-gated); B8 `unbxd` wasPrice + `is_available_state` flag-gated, `tests/test_exact_gate_flag_off_parity.py` proves flag-OFF==b207bfa. **GATES MET:** comm zero-regression (branch-only-NEW==[] vs 46 baseline, 8083 passed); 8 blockers red→green; rollback byte-identical (tested); no-fab (`guard_rejected` measured in metadata); **2 dispatcher-gated adversarial-review WORKFLOWS** — R1 found 6 leaks the self-probe missed (gender-flip/gift-set/Le-Parfum/accessory/whey-isolate/brand-abbrev), R2 found the OPPOSITE-error regressions R1's fixes introduced (gender de-padding over-rejecting Pour-Homme bestsellers; accessory pending genuine headphones/keyboards) — both fixed (gender→contradiction-only; accessory→narrow electronics-scoped `_is_device_accessory`). **LOAD-BEARING LESSON (why PR #9 was HELD + how it was beaten):** a green comm gate + a passing self-critique LIES — the original 110 tests encoded the wrong assumptions (missing-URL/unknown-stock "acceptable") and tested the helper (`is_exact_match`) the selector never runs. The antidote that WORKED: REPRODUCE every blocker through the public entry/the selector the orchestrator calls, and the DISPATCHER gates every review finding against real code (a single review pass would have shipped the gender/accessory over-rejections). [[feedback-green-gate-not-correctness]] [[feedback-verify-llm-reviewer-findings]] **DEFERRED to post-merge (the eval is a prod-HTTP harness): smoke20 vs `54b603e8` + flag-ON prod spot-check (`woo_store_api` Sauvage still the exact one) + the Serper-heavy COLD/WARMED `usable_exact_genuine`; warmer activation stays PAUSED until ≥85%/category.** Handoff: `memory/project_genuine_price_correctness_plan.md`.
