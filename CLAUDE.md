...
- `ENABLE_REGION_CURRENCY_GUARD` (default OFF, M13-11) — the response-chokepoint pend for a price whose currency ≠ the request region's currency (`response_builder` price-pending loop; region currency via `exchange_rate_service.get_region_currency`). **Effect ON:** a BHD price served to a `saudi_arabia` request pends (`amount None`, `unavailable True`, `guard_rejected=region_currency_mismatch` in metadata); a same-region price passes. **The highest-blast-radius change in the wave — canary it LAST, alone.** **Precondition:** flip WITH/AFTER `ENABLE_LONGEST_HOST_MATCH` — with the host-match OFF, `ksa`/`om.swissarabian.com` mislabel their own SAR/OMR as USD, so the guard would wrongly pend those correct storefront prices in a Saudi/Oman region.
