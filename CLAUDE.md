...
**Active runtime (genuine-price CORRECTNESS build — BUILT via ultracode Workflows, PR #9 OPEN + 🚨 HELD / DO-NOT-MERGE, 2026-06-27):** Implemented the planned 5-layer correctness on branch `feature/genuine-price-correctness` (9 commits off `b207bfa`; PR https://github.com/KGRddhs/smartcompare-backend/pull/9): A (55 TDD regressions) → B (`is_exact_match` strict-equality + `_selection_match` SUBSET-tolerant + `select_best` authority-not-cheapest + `is_available_state` + `is_price_showable(enforce_correctness=True)` chokepoint backstop + `ENABLE_EXACT_PRICE_GATE` rollback) → C (`_identity_cache_token` concentration+qualifier+size cache-key) → D (`usable_exact_genuine` KPI + non-circular truth set) → E/E2 (2 in-session adversarial-review Workflow rounds). **Comm zero-regression GREEN (8043 passed / 46 == prod baseline; +110 new tests pass).** **🚨 BUT an external deep review REFUTED the central "exact product, fail-closed, independently measured" claim — every blocker below REPRODUCED against the real code (gate-on). PR #9 IS HELD — DO NOT MERGE.** **LOAD-BEARING LESSON: the green comm gate + my own 2 self-review rounds LIED — the 110 new tests ENCODE the same wrong assumptions (they assert missing-URL + unknown-stock are "acceptable") AND never exercised the real runtime paths (iHerb/pharmacy/`select_best`). A green comm gate + passing self-critique is NOT proof of correctness — you must REPRODUCE the actual runtime leak. The strict matcher I unit-tested (`is_exact_match`) is NOT the one the selector runs (`select_best`→`_selection_match`); test the RUNTIME path, not the helper in isolation.** RELEASE BLOCKERS (all reproduced): (1) CRITICAL supplements return+cache the WRONG product — `fetch_iherb_price` (`price_service.py:~5408/5552`) falls back to best-overlap with NO threshold, strips the matched title, hardcodes `in_stock=True`; the pharmacy route calls `extract_jsonld_price()` WITHOUT `query_name` (`:5658`) → exact gate disabled (mocked Solgar D3 5000IU → Solgar Magnesium Citrate price). (2) CRITICAL `select_best` uses permissive `_selection_match` (query-SUBSET → flanker "Black Opium"→"Black Opium Over Red" ACCEPTED), and even `is_exact_match` STRIPS form tokens (`deodorant/candle/lotion/refill`, `:2794`) so a DEODORANT/CANDLE matches the perfume; a candidate OMITTING the query's stated concentration+size is auto-accepted (axes checked only when BOTH sides state them — fail-open). (3) CRITICAL the KPI is circular — `usable_exact_genuine_for_product(body, idx)` (`eval_runner.py:474`) NEVER loads the truth file it documents, no independent identity check, counts no-URL/unknown-stock as usable (`test_kpi_metric.py:47` codifies missing-URL OK). (4) HIGH `AggregateOffer.lowPrice` accepted as the exact variant (`:3928`), `priceValidUntil` ignored (expired-2020 offer shown), missing availability → `in_stock=True`. (5) HIGH missing identity+URL is FAIL-OPEN (a 400-BHD no-title-no-url `local_bhd` passes `select_best` + the backstop — contradicts the Cardinal Rule). (6) HIGH cache key is REQUEST-derived not RESOLVED-identity (`scs:4121`) — a wrong candidate that passes the permissive matcher caches under the requested product for the genuine TTL. (7) MEDIUM `guard_rejected` LEAKS into the public price payload (`response_builder.py:1249/1402` expose raw price dicts). (8) MEDIUM "flag-OFF byte-identical" is UNTESTED + false (no `ENABLE_EXACT_PRICE_GATE=false` test; keep-shopping-title / availability-semantics / unbxd-wasPrice-removal are unconditional). **LEGITIMATE: 110 new focused tests pass; full-suite 8043/46 == baseline (test-set zero-regression is real) — but the tests encode incorrect assumptions + miss the reproduced runtime paths.** **NEXT-SESSION FIX DIRECTIVE (Ahmed): fix iHerb/pharmacy bypasses FIRST; preserve candidate identity + real availability through EVERY adapter; require title/name + a valid PDP URL before selection OR cache write; REPLACE runtime subset matching with a STRUCTURED EXACT matcher that explicitly rejects flankers + different product FORMS (deodorant/candle ≠ bottle); a query-stated axis MISSING from a candidate = UNVERIFIED (pend), not auto-accept; `AggregateOffer.lowPrice` needs SKU/offer proof; REBUILD `usable_exact_genuine` to LOAD each truth entry + independently validate expected-identity / present-PDP-URL / confirmed-stock / current-offer; keep diagnostics in metadata NOT the public price; add the adversarial cases as FAILING tests, then rerun the comm gate + fresh cold/warm validation.** Handoff: `memory/project_genuine_price_correctness_plan.md`.
