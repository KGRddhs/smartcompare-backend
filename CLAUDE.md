...
**⚠️ CURRENT PROD STATE (2026-06-18 — READ FIRST):** walk-fixes SHIPPED (backend `2cb4439`, EAS group `3efa9d81`) from Ahmed's on-device walk — #20 FE results-organization (suppress `limited_data` dims + drop the price-derived 'value' dim on pricePending + single compact A·B bar legend + text-overflow fix) + #21 BE free KG/organic image tier (`image_service` Tier 1.5b `extract_image_from_search`, reads an image from the already-fetched unified Serper payload — $0, no Serper-Images budget, price-scrape-independent, strictly-additive). **🔑 THE SERPER KEY (`696e4e57…`) IS DEPLETED** (raw `/search` → 400 "Not enough credits") → **prod is DEGRADED** (no genuine specs/prices/images on a fresh compare) until rotated. **ROTATION = Ahmed's action** — new serper.dev key OR (recommended) PAID Serper (ends the recurring free ~2,500-one-time depletion + unlocks the warmer). Until rotation #21's fix is deployed but UNVERIFIABLE (every image tier needs the search). **[✅ ROTATION COMPLETE + VERIFIED 2026-06-18]** Rotated to a NEW key (`44e3b202…`, first-8): set on Railway production/web via CLI (`railway variables --set` + `railway redeploy`; Ahmed re-ran `railway login` to refresh the CLI's cached creds — the railway MCP stayed stale-token, the CLI worked as a fresh process) + LOCAL `.env` updated. VERIFIED LIVE via a prod `prices/iPhone+15+Pro?nocache=true` probe — Serper returning results (uae `gpt_organic_extract` + 4× `local_bhd` with real retailers; only bahrain → `estimated` = the known BH-feed structural gap, NOT a key issue). **PROD UN-DEGRADED.** #21 image tier now verifiable. New-key counter auto-fresh; optional minor follow-up: DEL `budget:serper:burn_alert_fired:*`. Full key lives ONLY in gitignored `.env` (never commit to a tracked file). **Railway-ops note:** when the railway MCP throws `Unauthorized`/`invalid_grant`, the CLI via Bash often still works (fresh process reads refreshed `~/.railway` creds) — try the CLI before declaring Railway unreachable; `railway login` itself can't run in the `!`-prompt (non-interactive) so the user must run it in a real terminal. **Two new durable gotchas:** (1) **stale-cache-masks-the-fix** — re-running the SAME pair serves the 7d-specs/24h-price cache (a pre-fix payload), so a "still broken" screenshot is usually stale cache NOT a failed deploy → re-test a FRESH/different pair or `?nocache=true` (the walk's "missing specs/wrong dims/no scent profile" was exactly this; a fresh pull confirmed the fix works). (2) **Serper headroom is POINT-IN-TIME, not sustained** — a clean N-query eval proves credits-at-that-moment, NOT headroom for the subsequent cumulative burn; near the cap heed the `budget:serper:<key8>:lifetime` COUNTER + track cumulative session burn (this session depleted a near-cap key by over-trusting one clean eval + dismissing the canary). **What's left:** **[SHIPPED 2026-06-18]** the FE polish wave — W1 (CategoryProfile → INSIDE the "Dig deeper" accordion as the first "At a glance" section, per Ahmed: a standalone block confused the pre-compare category pick), FA4 (fashion labels = clearest one-line forms EN+AR), AND the richer structured "where the runner-up wins" CARD (`RunnerUpWinsCard.tsx` — real runner-up-leading dims only, no point-math, gray-not-emerald, hides-when-empty, symmetric) — all gated diff-by-diff, merged `main 9172e62` + EAS preview `96098d43`. **Serper ROTATED + VERIFIED (above) → prod un-degraded.** **[CODE-REVIEW 2026-06-18]** Codex full-arc review (`3d870c8^..HEAD`, via the `codex:codex-rescue` agent) → 5 fixes shipped (backend `ff83298`+`157afff` Railway / FE EAS preview `1789ec22`): SF-1 (`converted_usd` no longer 30d-negative-cached so the scrape cascade / warmer can upgrade it to a genuine BHD price — **resolves the warmer's negative-cache prereq**), SF-2 (Dig-deeper Specs-table keys via `t()` → Arabic labels), SF-3 (`_BARE_DOMAIN_RE` anchored to real TLDs so praise keeps "S24.Ultra"-style product strings), N-4 (`extract_image_from_search` adds a top-level Serper `images[]` fallback), MF-1 (longevity-tie strict `>` — verified NO-OP, cosmetic). **DURABLE LESSON — gate every Codex/LLM-reviewer finding against the real code:** the lone "MUST-FIX" (MF-1) was a verified no-op and a "nit" (N-1) named a non-existent symbol — ~half the actionable findings weren't real; re-derive every must-fix + grep every named symbol before "fixing" it (`memory/feedback_verify_llm_reviewer_findings.md`). Codex DID confirm the no-fab invariants + cache-TTL architecture + cohort wiring sound, and SF-1 was a genuine architectural catch — worth running. Full triage: `memory/project_faithful_results_shipped.md`. REMAINING: Backend's #21 image-tier prod confirm (now unblocked); Ahmed's on-device re-test (FRESH pairs, EAS two-launch); the warmer cron + `ENABLE_PRICE_CACHE_WARMER` (needs paid Serper for sustained). Still offered-not-built: a deploy-version cache-bump. The 4-Opus `faithful-results` team is on STANDBY (disassembly = Ahmed). Full handoff: `memory/project_faithful_results_shipped.md`.

**Active runtime (catfix — category resolution + all-9 render correctness, SHIPPED 2026-06-21, main `698006a`):** 4-Opus worktree team + dispatcher (one clean merge over base 5eef9d9; EAS preview `0c39fc44`). Keystone: resolve the pair category + write it onto `products[i]["category"]` BEFORE the `_fetch_product_data` gather (sync+stream) via the shared `_resolve_pair_category` helper. An adversarial **ultracode Workflow review run BEFORE the prod ship caught a real HIGH regression the TDD team + cross-QA had missed → FIX-1:** on the q= parser path the LLM-emitted per-product category is now AUTHORITATIVE (was: the deterministic `classify_category_from_text` keyword classifier overrode the LLM → 4/200 gold queries misroute, e.g. `Tefal steam iron`→supplements via `is_supplement_query('iron')`, `Vitamin C serum`→supplements, `True Skin concealer`→skincare, `food container`→grocery; INVISIBLE to the smoke20 subset, so a green smoke20 is NOT proof for this class — unit-pin instead); name-detection stays authoritative for explicit_pair/vision (threaded `parser_path = not (vision_products or explicit_pair)`). **FIX-2:** the A5 `rating_derived` guard was missing on the SSE intermediate `reviews` event (latent NO-FAB leak + a false code comment) → now mirrors the response_builder projection. + polish: deleted prod-dead `resolve_category` (its 6 uncovered precedence cases PORTED to `_resolve_pair_category` tests first — the two ladders DIVERGED after FIX-1); URL-compare canonical enum + category routed into `generate_comparison`; `classify_category_llm` wrapped in `asyncio.wait_for`; partial-ctx category seed; api.ts SSE-fallback gated. Verified: free-unit gate (48 pre-existing reds byte-identical, ZERO catfix-introduced; the documented order-flaky `test_rate_limiting_complete` is the only extra); D2 cold prod probe GREEN (`category_used=fragrances`, real ratings, genuine prices); smoke20 ACCEPTED (winner FLAT / factual HELD / 0 estimates / 0 errors; specs 0.925 vs baseline 0.9875 = cold-run/timeout noise — the "GATE FAIL" was a stale-OS-`SUPABASE_*`-env shadow, not a regression). **⚠️ ON-DEVICE 2026-06-21 — Ahmed's screenshots were LARGELY STALE CACHE (the stale-cache-masks-fix gotcha; this exact pair was cached pre-fix). A FRESH `nocache` pull of the CURRENT 698006a backend (Tom Ford Oud Voyager vs Oud Wood, q=) is the ground truth. The category fix WORKS (renders fragrance). STALE/GONE in current: the "94.0 BHD" price contradiction, the raw "49.8" score, the 70.69 price (now 146.64 converted_usd), "6-8hr" longevity (now 10-12). CURRENT real content issues (NOT catfix regressions; = next "fragrance content quality" bundle): (1) RAW SCORE LEAKS in verdict/pros TEXT — recommendation "wins with a 10.7-point higher overall score" + Oud Wood pros "Strong presentation score of 100" (no-internals-in-reveals; overlaps the Bundle-C A.10.x verdict-forbidden-words audit — find the gap); (2) ASYMMETRIC pricing — Voyager 146.64 BHD shown, Oud Wood pending (reason=pending_genuine = no genuine BH source) → structural BH-source gap + poor UX; (3) SOFT price-ref while pending (Oud Wood cons "premium price point" while pended); (4) metadata.category_used=None on this q= pull (vs D2=fragrances) — possible partial-response artifact (fragrance compares near the 30s cap) or a q= metadata gap; the CONTENT is fragrance. Reviews carry real ratings+sources now (mercari via Google Shopping). LESSON (TWICE this session): I diagnosed from a screenshot AND a top-level HTTP-200 probe instead of the ground truth — the FRESH `nocache` pull with the actual content inspected is the ONLY reliable check (stale-cache gotcha + verify-the-RESULT-not-a-glance). ⇒ next "fragrance content quality" bundle (overlaps deferred G1 + URL-compare full scoring + the prior Thrust-C `115244b`/faithful-results F1.2/`review_praise`/A.10.x content work — check those for regressions/gaps).** DURABLE GOTCHA: stale OS `SUPABASE_*` exported in the User-scope Windows env pointed at a DELETED project → shadowed `.env` (`load_dotenv()` no-override) → broke local eval persist/baseline-fetch (NOT box-DNS, NOT a regression); removed from User-scope 2026-06-21 (restart Claude Code to shed the current session's inherited copy). Full handoff: `memory/project_catfix_shipped.md`; **NEXT-BUNDLE PLAN (fragrance content quality — score-leak-in-text, price-pending↔verdict consistency, category_used=None, + deferred G1 + URL-scoring; full session findings + ready-to-paste kickoff): `docs/plans/2026-06-21-fragrance-content-quality-bundle.md`.**
