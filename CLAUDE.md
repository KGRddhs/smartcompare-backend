...
**Active runtime:** **Sprint A Backend Comparison Engine Overhaul + B0 Hardening shipped 2026-06-09 (main `3f4f8d1`).** 4 Sprint A lanes + 3 B0 hardening lanes + B0-E polish + B0-A v2/v2.1/v2.2 phantom-tie closure. 24-query bias matrix: 91% RED → 0% RED across 3 verification phases. Sentry 4→1 unresolved (PYTHON-FASTAPI-J/6/K all resolved; -9 single-event auth refresh defer-and-monitor). Bundle B kickoff prep at `docs/plans/2026-06-09-bundle-b-kickoff-prep.md`. **Pre-Sprint-A:** Bundle E S3 Hot-Fix Wave 2 merged 2026-06-03 (`6d73b6a`) closed device-walk Wave 1 RED items. Latest `eas update` group `90087c4f-ee62-4e4c-84e7-d0c17a62276f` on `preview`. `STREAM_HARD_CAP_SECONDS=30.0` (raised 25→30 in B0-C Item 3, 2026-06-09). `ENABLE_FIRECRAWL/SCRAPEDO/PAGE_SCRAPE/DEBUG_STAGE_TIMINGS=true`. `SERPER_API_KEY` rotated to `4ab4ec...` 2026-06-09 (prior key depleted, Sentry-6/K resolved).
