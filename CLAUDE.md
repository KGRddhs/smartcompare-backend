...
Applied 010–022 (all via MCP since 013). Migration files in `migrations/*.sql` with rollbacks at `migrations/rollback/*.sql`. Highlights: **018** bonus expiry columns + partial idx. **019** `users.attribution_source` + CHECK enum mirror. **020** `comparisons.schema_version` (v1=legacy/hidden, v2=renderable; default 2). **021** `users.device_fingerprint_hash` + partial idx (anti-farming). **022** `referral_invites.source` + relaxed `comparison_id` + CHECK `source∈{share_link,code_redeem}`. Bundle B/C/D adds **023** (lifetime_invites_consumed; see in-progress section below).
...
### Bundle B/C/D Consolidated (in progress — Session 46, 2026-05-12)
Polish + referral hardening shipping via 4-Opus worktree team. Worktree: `../smartcompare-bundle-bcd` on branch `feature/bundle-bcd`. Design + plan: `docs/plans/2026-05-12-bundle-bcd-consolidated{,-design}.md`.
- **Scope (8 items):** Arabic AI-assisted proofread, Cal-AI fullscreen camera redesign (reticle + dual slots + gallery picker), type-input "need TWO" hint, category chip emoji→lucide glyphs, header `<QarenLogo />` SVG, Cal-AI animation polish, referral hardening (see below), perf audit + obvious-win fixes.
- **Branch.io DROPPED** mid-design — free tier paywalled to $199/mo. Replaced with hybrid DIY: Android Play Install Referrer + iOS clipboard fallback + Cloudflare Worker at `qaren.app/r/{code}`. Modules: `playInstallReferrerService.ts`, `clipboardFallbackService.ts`, `attribution_service.py` (backend).
- **Referral model overhaul (Migration 023):** drops `users.weekly_invites_used`; adds `users.lifetime_invites_consumed INT DEFAULT 0` + partial index on `device_fingerprint_hash`. Cap is **3 LIFETIME per device** (cross-account via fingerprint SUM), decrement at **receiver signup** (not share), share-button **disabled at 3 lifetime**, bonus expiry **7 days** (was 3 from Migration 018; existing rows unchanged).
- **Cloudflare infra (Session 46):** qaren.app DNS migrated from Name.com → Cloudflare nameservers. Cloudflare Email Routing live for `ahmed@qaren.app` + `support@qaren.app` → `kingzatel@gmail.com` (handles SRS properly where Name.com forwarding failed Branch's DMARC). Cloudflare Worker for `qaren.app/r/{code}` web fallback deploys in Phase 3 (Ahmed runs `wrangler login` + `wrangler deploy` interactively).
