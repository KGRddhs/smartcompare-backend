...
**Ready-to-paste NEXT-SESSION prompt (genuine-price warmer + variant metadata — ultracode TEAM):**
```
ultracode. The genuine-price exact-SKU gate is SHIPPED to main (cdaf5c5) + deployed. Execute the brainstormed plan in
docs/plans/2026-06-30-genuine-price-warmer-and-variant-metadata-design.md as 2 sequential ultracode WAVES, each:
recon → TDD implement (sequential writers, path-restricted commits) → adversarial COVERAGE-DRIVEN sweep (enumerate the
space, reproduce through the runtime selector — the method that caught what 14 hypothesis reviews missed) → dispatcher
gates every finding → comm gate (worktree of origin/main + .env + free-unit suite both sides + comm -13; branch-only-NEW
must be [] vs .qa-correctness/main-baseline-failed.txt; CI backend-tests is RED-by-design — gate on the comm-diff, not CI).

WAVE 1 = warmer + measurement, GATE-FIRST (the warmer AUTO-ACTIVATES iff the gate passes, so build a gate trustworthy
enough to hand it the prod-cache-write decision). 3 legs: (a) a reliable per-category WARMED usable_exact_genuine KPI
(binary; ≥85%/cat unlocks; baseline anchor is the FULL uuid 54b603e8-4eab-41c9-a34d-a5e391446559, NOT the truncated
54b603e8; the smoke20 winner axis is ±0.10 noise — do not gate on it); (b) cache-key PARITY (derive the key from the
RESOLVED-match identity not the raw query, else warmed "Eau de Toilette" ≠ live "EDT" = guaranteed miss); (c) fresh-purge
precondition; + fold in the titleless-price display/cache consistency + a Serper-budget guard. Flip ENABLE_PRICE_CACHE_WARMER
ON iff KPI ≥85%/cat ∧ cache-parity proven ∧ clean cache. GOTCHA: a local _get_price/warm with nocache=True STILL WRITES the
shared Upstash + product_prices DB (nocache bypasses the READ, not the WRITE) → local testing pollutes prod cache.

WAVE 2 (only after Wave 1 gate passes) = a structured VariantDescriptor (option C: formalize the existing axis extractors
+ a NARROW LLM hint only where axes can't decide) to close the ~9 token-indistinguishable residuals (gender flanker,
one-sided SPF, same-token concentration flanker Sauvage→Elixir, cross-unit g↔ml, makeup one-sided formula, ZMA/Cal-Mag
acronyms) — every token fix re-introduces a WORSE over-rejection, so compare structured fields, not strings. Flip the
xfail/held pins in tests/test_correctness_coverage_sweep_fixes.py where the descriptor decides them.

Full context: the design doc above + memory/project_genuine_price_correctness_plan.md.
```
