...
**Security** (`app/utils/`): `url_validator.py` — SSRF: resolves hostnames, blocks private/loopback/link-local IPs, allows only http/https. `prompt_sanitizer.py` — prompt trust boundary (M18 PO-prompts-04/05, unflagged): `sanitize_prompt_input` neutralizes literal `<USER_INPUT>`/`<SEARCH_RESULTS>` region tags (case/whitespace-tolerant, `</USER_INPUT>` → `[/USER_INPUT]`) so a crafted query can't close its untrusted region; the Serper title/snippet digest reaches the specs/price/reviews prompts ONLY inside a `<SEARCH_RESULTS>` region (`extraction_service._wrap_search_context` chokepoint + per-snippet `sanitize_untrusted_block` in both `structured_comparison_service` formatters — `raw_snippets`/`_search_snippets` are deliberately the SANITIZED text so fact-check compares what the model saw), and all four consuming system prompts carry the `SEARCH_RESULTS_GUARD` do-not-follow sentence. Pinned by `tests/test_prompt_fence.py`. Known residual: `_smart_fallback_extract` → `openai_service.extract_specs_targeted` still interpolates snippets without a region (out of the M18 finding's scope).
...
Every product has `fact_check` with per-field counters only — `specs_verified/likely/flagged/unverified`, `price_verified`, `price_deviation_pct`, `review_sentiment_consistent`, `review_rating_deviation`. **There is NO `overall_confidence` field** — Bundle E Decision 7 dropped it (tests pin its absence); user-facing confidence is `compute_confidence`'s per-leg pills + `overview.confidence.overall`, which do not read fact_check unless `ENABLE_CONFIDENCE_FACTCHECK_WIRING` (#109) is on. Spec citations verified vs snippets; review sentiment vs Serper (0.8 tol); price vs Shopping median (30%). Zero extra API calls. Absence is reported honestly: `review_sentiment_consistent: None` when nothing to compare (a string `average_rating` is coerced at parse time AND in `verify_review_sentiment` — M18 PO-fact-check-09 crash fix), and behind `ENABLE_FACTCHECK_HONEST_ABSENCE` (default OFF, M18 PO-fact-check-07) `price_verified: None` when zero shopping rows exist to check against (flag OFF keeps the legacy fabricated `True` + its +0.1 reliability bonus; estimates stay `False` in both modes). Both products' fact_check ship at `metadata.fact_check.product_0/1`, and the Decision-7 notice finally has a producer (M18 PO-fact-check-10): `metadata.data_freshness_shaky` = `is_data_freshness_shaky([fc0, fc1])`, attached unflagged (additive key) by `structured_comparison_service.attach_data_freshness_notice` on all three build paths. **Ratings are NEVER AI-generated** — structural since M18 PO-fact-check-11: `_normalize_review_response` unconditionally clears any model-emitted `source_ratings` (real retailer ratings are injected afterwards by `review_service`); the prompt prohibition remains as defense-in-depth.
...
**M18 load-foundations + scoring-math flags (`#100`, `#101`, `#115`, `#116`, `#117`, merged 2026-09-02).**
- `ENABLE_HOME_SAVINGS_AGGREGATE` (default OFF, `#116`) — replaces the unbounded `/home/savings` scan (which SELECTed every row's `full_response` JSON and aggregated in Python ON the event loop) with ONE `home_savings_aggregate(p_user_id)` RPC. **PRECONDITION: migration `036` must be APPLIED before flipping** — it is written but NOT applied (like 033/035), and the function is inert until then; flag OFF keeps the legacy Python path. **`036` carries a CROSS-TENANT GUARD added in review before it was ever applied:** the function is SECURITY DEFINER (bypasses RLS) and GRANTed to `authenticated`, and PostgREST exposes every GRANTed function as `/rpc/<name>` — so "the route always passes the authenticated user's id" was not a control. The WHERE clause now re-derives the caller (`auth.uid() = p_user_id`, service_role allowed explicitly); a cross-tenant call returns zero rows. `tests/test_migration_index_predicate_immutability.py` pins this for ANY future SECURITY DEFINER `p_user_id` function (comment-stripped, so prose cannot satisfy it).
- `OPENAI_MAX_RETRIES` / `OPENAI_FALLBACK_MAX_RETRIES` (env knobs, not flags; defaults 2 / inherit = byte-identical unset, `#117`) — explicit retry ceilings on all four `AsyncOpenAI` constructions so a 429 storm cannot multiply requests. `model_config.verdict_chain_max_attempts()` makes the worst case derivable (6 shipped default, 3 at the runbook launch values). Sizing arithmetic: `docs/runbooks/2026-09-02-openai-tpm-launch-sizing.md` — every derived figure labelled **MODELLED**, and the ~6.6-10 compares/min ceiling is CONDITIONAL on an UNVERIFIED Tier-1 assumption (verifying the real tier is a hard launch gate).
- `ENABLE_SPEC_FIELD_NORM` (default OFF, `#100`) — normalizes spec fields PER FIELD before aggregating instead of summing raw cross-unit magnitudes (a 5000 mAh battery, a 6.1 in display and 128 GB storage were added as bare numbers, so the biggest-magnitude field decided the dimension and a product could win on unit choice). A `>=10x` same-field gap ties at the midpoint as a unit/notation artifact. **Known residual:** that tie also erases a GENUINE same-unit 10x gap (16 GB vs 512 GB) — canary-watch.
- `ENABLE_MISSING_DIM_RENORM` (default OFF, `#101`) — renormalizes over dimensions actually present instead of injecting `MISSING_SCORE=50`, which let a product with NO data outscore one with genuinely poor measured data. **Two P1 inversions were found in review and fixed before merge** (the flag reintroduced the bug it targets): (a) renorm excludes exactly the dims the measured product wins, leaving parity, and the price-authority delta then penalised ONLY the product with data because an all-missing product was EXEMPT — price authority is now SYMMETRIC under this flag; (b) the effective set could reduce to nothing but both-sided-missing sentinels, so ZERO measured data decided the winner — that now falls back to the legacy sum. Parity ties prefer the product with MORE measured dims instead of argmax-on-index. **Known residual (canary-watch):** a mixed-provenance pair (measured product on an ESTIMATED price vs a rating-void on a TRUSTED price) still inverts — that is the authority feature doing its designed job, and closing it needs a redesign that abandons exclude-and-renormalize.
- **`CATEGORY_MIN_COVERAGE` is NOT inverted** — the M18 report claimed it was; verified against the code, `penalty_factor = 0.5 + coverage_ratio` applies only below `min_coverage` and INCREASES with coverage, so lower coverage takes the LARGER discount. The real "low coverage wins" effect is magnitude concentration, which is what `#100` fixes. No change made.
...
- `ENABLE_SYNC_DB_OFFLOAD` (default OFF, M13-05) — `app/utils/db_offload.run_db(call)` moves the REQUEST-PATH HOT SET of blocking Supabase `.execute()` off the loop via `asyncio.to_thread`. **Effect ON:** one user's blocking DB RTT no longer stalls every other coroutine on single-worker uvicorn. **Hot set (9 sites):** `database_service.{get_user_by_id,get_user_comparisons,save_comparison}`, `usage_service.{_get_user_tier_info,_get_active_referral_bonus,record_comparison(increment rpc),record_lifetime_comparison(rpc)}`, `auth_service.verify_token`, `audit_service.log_audit_event`. **Flag OFF:** `run_db` is an `async def` with no `await` on the inline branch → byte-identical to a direct `.execute()`. **Precondition:** none; canary alone. **Deferred list CLOSED 2026-09-02 by M18 `#115`** — every site named here (`usage_service._maybe_reset_referral_bonus`, `_persist_genuine_price` set_cached, `database_service.log_search`, `feedback_service.save_comparison_and_track_cohort`) is now wrapped, plus the product_data L2 executes, the auth/behaviour-profile reads and the referral Loop-2 entry queries. Still deliberately left: off-path/cron/script sync-Supabase sites (off-clock by design), `usage_service.get_usage_status`/`check_usage_allowed`, `review_service` L1 get/set, and the behaviour-profile RPC (needs migration 037).
