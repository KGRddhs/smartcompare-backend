...
- **Bundle A deploy status (as of 2026-05-12):** ✅ PR merged, ✅ local main synced, ✅ Railway backend live (`/health` 200). ☐ `eas update --branch preview` PENDING. ☐ `eas update --branch production` PENDING. Mobile testers run last-bundled JS until EAS fires — Bundle A frontend (4 new screens, deviceFingerprint, ToggleRow, locale-aware i18n) is invisible to phones today. Owned in next session.
- **GitHub OAuth posture (2026-05-11 incident):** during PR #3 push, GitHub token leak triggered a full OAuth-app sweep. **Revoked from KGRddhs GitHub account: Visual Studio Code, GitHub CLI, Supabase.** Only Git Credential Manager kept (required for `git push`). Supabase MCP uses service keys, not OAuth — unaffected. **Restoring VS Code + Supabase OAuth + reinstalling `gh` is owned in next session.**
