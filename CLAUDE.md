...
### Bundle history (sessions 44-49)
See `docs/SESSION_BUNDLES.md` — Bundles A (PR #3), B/C/D (PR #4), E (PR #5 + Session 48 merge `e67d583`), **Session 49** (D1 scatter-gather shipped + Bucket A 4-bug fix + D2 design+plan committed but implementation deferred). **Active state:** Bundle E + Session 49 EAS group on `preview` is the latest `eas update`. `STREAM_HARD_CAP_SECONDS=25.0` locks streaming p95 ≤25s. `SCRAPING_MODE=soft` URL gate **wired** at Firecrawl + Scrape.do call sites. **Pending next sessions:** (1) **D2 Section 3 implementation** per `docs/plans/2026-05-17-d2-mainstream-speedup.md` (Phase 1 collapse + OpenAI prompt caching; target cold p50 ≤15s; 5-category parameterized test suite); (2) **Bucket C brainstorm** re-scoped from real post-D2 behaviour (some C items may auto-close from D2's prompt restructure); (3) **Bucket B brainstorm** dedicated UX session (two-input text/URL boxes redesign). **Worktree-team workflow:** `git worktree add -b feature/<name> ../smartcompare-<name> main` → 4-Opus TeamCreate (`mode: "bypassPermissions"` REQUIRED — sandbox blocks Bash otherwise) → cross-QA → merge `--no-ff`. **Arabic-as-default DROPPED** (Session 44).
