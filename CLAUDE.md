...
- `source_method` enum (re-enumerated from the stamp sites 2026-09-01, M20 #104 — the old 7-value list was stale). **Genuine BH (`price_service._GENUINE_BH_SOURCE_METHODS`, 15, and since #104 == `scoring_service._PRICE_TRUST_SET`):** `local_bhd | page_scrape | page_scrape_jsonld | page_scrape_rendered | shopify_json | official_brand | firecrawl | firecrawl_brand_domain | scrapedo_rendered | woo_store_api | salla_api | occ_rest_bhd | magento_graphql_bhd | rest_json_bhd | zyte_render_bhd`. **Not genuine:** `converted_usd` (a real figure but not local data — deliberate HALF authority penalty), `estimated` (Tier-3 GPT training estimate — full penalty). **Sentinels, never a price:** `sitemap_no_match`, `validation_rejected`. `confirmed_multi_source` is `quality_ranker.select_best_price`'s agreement stamp and has **no live call site** — it never reaches `product.price.source_method`, so it is deliberately outside both sets. `price_method_mismatch` flag set when products differ.
...
- `ENABLE_REGION_CURRENCY_GUARD` (default OFF, M13-11) — the pend for a price whose currency ≠ the request region's currency (region currency via `exchange_rate_service.get_region_currency`). **Runs in TWO places since M20 #113:** (1) `price_service.apply_region_currency_guard`, an in-place PRE-SCORING pass called from both orchestrator paths right after `reconcile_pair_fairness` and before the `specs`/`prices` yields and `compute_scores`; (2) the original `response_builder` price-pending loop, kept as the idempotent backstop for direct `build_comparison_response` callers (tests, share/history rebuilds). **Effect ON:** a BHD price served to a `saudi_arabia` request pends (`amount None`, `unavailable True`, `guard_rejected=region_currency_mismatch` in metadata) — and, since #113, it pends BEFORE the winner is scored and before the SSE `prices` event ships it, so neither `winner_index` nor the LLM verdict nor the streamed card is decided by the mismatched amount; a same-region price passes. **The highest-blast-radius change in the wave — canary it LAST, alone.** **Precondition:** flip WITH/AFTER `ENABLE_LONGEST_HOST_MATCH` — with the host-match OFF, `ksa`/`om.swissarabian.com` mislabel their own SAR/OMR as USD, so the guard would wrongly pend those correct storefront prices in a Saudi/Oman region.
...
**ACTIVATION coupling (Wave-2):** the four currency flags are canaried TOGETHER as a family, but with two hard orderings — (a) `ENABLE_EXTENDED_FALLBACK_RATES` with/before `ENABLE_SHOPPING_STRICT_CURRENCY` (else TRY/PLN/CAD ASCII-ISO shopping rows still ship mislabelled), (b) `ENABLE_LONGEST_HOST_MATCH` with/before `ENABLE_REGION_CURRENCY_GUARD` (else the guard pends correct swissarabian SAR/OMR prices) — and **`ENABLE_REGION_CURRENCY_GUARD` goes LAST, in its own window** (highest blast radius: in any non-Bahrain region it pends every non-region-currency price). **SECOND HARD PRECONDITION added 2026-09-01 (M20 #113): do NOT flip this flag on any build that predates the pre-scoring pass.** Before #113 the guard pended only in the FINAL projection, so a final-payload canary read clean (`price: pending`) while the winner had already been chosen from a 10-BHD-vs-40-SAR comparison and the SSE `prices` event had already flashed the raw foreign amount — the half that matters would have gone unnamed. Remember M13-07/08/40 are already live on deploy via default-ON `ENABLE_JSONLD_FIRST`; the only kill-switch for them is disabling the whole Wave-1 JSON-LD reorder.
...
**M13-35 resource note:** drain-not-abandon runs the full Phase-2 OpenAI tail server-side even when the client disconnects BEFORE the verdict, and that tail is default-UNBOUNDED (`ENABLE_FULL_STREAM_DEADLINE` default OFF). An early connect-then-drop now amplifies into a full server-side compare where it previously self-cancelled. **CORRECTED 2026-09-01 (M20 #112):** this was NOT a pure resource trade-off — the same drain also silently BURNED one of the user's 3 daily / 10 monthly / 3 lifetime free comparisons for a result they never received, because the metering branch was reached instead of the refund. M20 #112 fixes the accounting half UNFLAGGED and gates the drain on `complete_response is None` behind `ENABLE_PREVERDICT_DISCONNECT_ABORT` (default OFF) — exactly the "gate the drain" option named here.

**Wave M20 flags (output truth — the winner card, the SSE verdict, the pre-verdict disconnect, the region-guard ordering; branch `feature/m20-output-truth` from `17cb981`, 2026-09-01).** M18 findings `#99, #104, #110, #112, #113`. Every new flag is default OFF and read PER CALL via `os.environ`/`os.getenv`.
- `ENABLE_WINNER_PROSE_RECONCILE` (default OFF, #110) — on a deterministic-vs-GPT winner disagreement, REPLACES the three verdict strings with the deterministic template (`response_builder.deterministic_verdict_fields`: `winner_declaration = <winner name>`, `winner_reason = "<winner> edges ahead on the overall picture."`, `key_tradeoff = "<loser> stays competitive on <dim>."`). **Effect ON:** user-facing verdict COPY changes on the mismatch rows — canary it ALONE, it must not move with another flag. **Flag OFF is NOT a no-op** and deliberately so: the UNFLAGGED safety half of #99/#110 still runs (see below).
- `ENABLE_PREVERDICT_DISCONNECT_ABORT` (default OFF, #112 Half B) — a pre-verdict SSE disconnect stops driving the orchestrator and CLOSES its async generator (`contextlib.aclosing` → `aclose()` → GeneratorExit → the orchestrator's finally + M13-30's `_get_price` prefetch cancellation run deterministically; a bare `break` would NOT do this). **Effect ON:** the default-unbounded verdict/critique/moderation OpenAI tail is not paid for a comparison nobody will read. **Flag OFF:** the `break` never fires and M13-35's drain is unchanged, byte-identical. **Precondition:** none; pairing with `ENABLE_FULL_STREAM_DEADLINE` is complementary, not required.
- **UNFLAGGED (#104 — scoring truth):** the 8 genuine-BH stamps missing from `scoring_service._PRICE_TRUST_SET` were added, so the two sets are now EQUAL (the pinned invariant stays a SUBSET — a future scoring-only member must not redden a price_service test). A genuine `page_scrape_jsonld` Bahrain price no longer takes the −4.0 estimate-grade authority penalty (which was HARSHER than `converted_usd`'s −2.0), renders the price leg **strong** with zero shopping rows, and can be cited in winner evidence. Unflagged because the change is one-directional — it can only remove a penalty from a price the rest of the system already calls genuine; winner flips where one side is genuine-scraped and the other estimated are the documented "facts beat estimates" intent, not a regression. `tests/test_price_trust_set_parity.py` is the anti-drift pin.
- **UNFLAGGED (#112 Half A — accounting):** a new `complete_after_client_gone` latch records whether the FIRST final payload was captured while the client had already left, and the metering branch is gated on it. A pre-verdict departure now refunds via the existing `usage_refund.text_stream.incomplete` label and writes no `log_search` success row, no `save_comparison`, no `record_lifetime` — net quota effect zero, as it was pre-Wave-3. A POST-verdict departure still meters and persists exactly once (M13-35's contract, pinned). Unflagged because it is a pure defect correction with no result fork for legitimate completed traffic; shipping it dark would leave a live, repeatable credit burn behind a flag nobody has flipped.
- **UNFLAGGED (#99 + the index/name half of #110):** `response_builder.reconcile_winner_prose` is now the single winner chokepoint for BOTH the sync and the streaming path. On a mismatch it re-indexes the BC `comparison["winner_index"]` alias, drops GPT's `winner_declaration` (so `overview.winner.name` falls back to `product_names[winner_index]`), drops GPT's `key_tradeoff` (written from the INVERTED orientation — it frames the shipped winner as the runner-up), and drops a `winner_reason` that contains the LOSER's **full** name (case-insensitive containment, never token matching — the two products usually share a brand token) so the existing qualitative fallback fires. The SSE `verdict` event now carries the same reconciled index/name, closing the mid-stream winner FLIP between `verdict` and `complete`. **Why unflagged:** `winner_index` is ALREADY deterministic in prod, so the only rows this can touch are rows that are already self-contradictory — shipping the LOSING product's name in the headline and the Share text is indefensible and must not wait on a flag flip. The agreement path (the common case) is untouched. It owns the `WINNER_INDEX_MISMATCH` warning, which now also logs both product names and still fires **exactly once per request** (the repair is idempotent, so the second call from `build_comparison_response` is a no-op).
