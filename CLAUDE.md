...
Applied: 010 (RLS), 011 (freemium + audit log), 012 (product data tables), 013 (demographics + cohort views, applied 2026-05-05 via MCP after a 2026-05-04 SQL Editor rollback), 014 (referral system), 015 (push tokens), 016 (referral invite privacy JSONB), 017 (share_token VARCHAR→TEXT), 018 (referral_redemptions.expires_at + expiry_reminder_sent_at + referral_invites.deep_review_expires_at + partial-WHERE index, 2026-05-07 via MCP — verified live), 019 (users.attribution_source TEXT NULLABLE with CHECK enum mirror, 2026-05-07), 020 (comparisons.schema_version INT — v1=legacy/hidden, v2=renderable; default 2 for new inserts, 2026-05-11 via MCP), 021 (users.device_fingerprint_hash TEXT + partial index, 2026-05-11 via MCP), 022 (referral_invites.source TEXT + relaxed comparison_id NOT NULL + CHECK source∈{share_link,code_redeem}, 2026-05-11 via MCP — unplanned but required because design assumed schema that didn't exist).
...
### Sharing + History
22-char URL-safe token (`token_urlsafe(16)`) in `comparisons.share_token` (TEXT post-migration 017 — was VARCHAR(12), causing silent insert failures pre-fix in commit 0b01d9a). Public access strips personalization. History: paginated, searchable, ownership-checked. On 401, clears session + redirects to auth. `create_share_token` raises `ShareTokenError` on persistence failure (loud-fail). **schema_version gate (Migration 020, Bundle A):** `save_comparison` only writes when `_validate_renderable(payload)` passes; sets `schema_version=2` + denormalized `product_names`. History list/count/get filter on `schema_version=2` — v1 rows invisible (use `include_legacy=True` for DELETE cleanup only).

### Qaren UX Redesign (Phase 5 merged 2026-05-07 — PR #2 ee91a87)
Cal-AI-Lite 17-step onboarding + black/emerald hybrid identity (emerald = signal color reserved for winner reveal, success ticks, cohort accents, the one-time invitee "See how it scores for YOU" CTA — NOT primary CTA). `CANARY_NEW_ONBOARDING_PERCENT` lives in `SmartCompareApp/src/config/features.ts`; **currently 100 for build/test mode (commit 462b399)**. Drop back to 10 via EAS Update before App Store soft-launch, then ramp 10→50→100 (NOT a Railway env var). Plan/spec at `docs/plans/2026-05-06-qaren-ux-redesign{,.design}.md`. Session 43 in `CONTEXT_SESSION_LOG.md`.
- **Bucketing (`featureBucket.ts` + App.tsx):** djb2 hash on stable id (device-id pre-signup via `expo-secure-store`, user.id post-signup). `hashBucket(id, percent)` is pure — same `(id, percent)` → same boolean every call, every device. Monotonic ramp invariant verified by tests. `getStableId()` resolves BEFORE onboarding-render state setters; after auth, `setStableUserId/setFlagStableId(authUser.id)` re-bucket on user.id for cross-device consistency.
...
- **Hero illustrations + reusable components** (hand-coded SVG + Reanimated, ZERO Lottie/JSON; ~0 bundle impact): PhoneMockup (Figma placeholder, testID preserved), CohortBarChart (4 bars + 388-dot grid + spiral peer cluster), ConcentricMotif, LoadingRings (screen-14 centerpiece), RevealBurst (8 lines + Q-badge + check). Components: CounterTicker (safety-floor for jest+on-device parity), StageChecklist (✓/⟳/○ with haptic on transition INTO done only — never on initial mount), ProgressBar (`variableEasing` 4-segment fast/slow/fast/snap), StreamingProductCard (stage-gated SSE: init→title→specs→prices→reviews→verdict), LoadingTipsCarousel (5 tips after 8s), CohortBadge (RTL-aware slide).
...
- **Onboarding analytics (`OnboardingFlow.tsx`):** `onboarding_started` on mount, `onboarding_step_completed` on every Continue (BEFORE setStep so payload reflects FINISHED step), `onboarding_completed` on Step 17. Payload `{step_number, step_name, locale, flow_variant}`; `flow_variant: "new"` locked at mount. Legacy symmetry = Task #60 before #48 ramp.
- **Copy contract:** ZERO scary copy in user-facing i18n. Forbidden = anything implying error/failure/lockout (`couldn't`, `try again`, `Failed to`, `تعذر`, `فشل`, etc.). Approved vocabulary: "Hold on — X. Tap to retry.", "Reconnecting…", "Sharper match coming up", "expired or moved", "are paused right now". Full list in design doc §6.
...
- **Admin:** `/admin/referrals/*` + `/admin/costs/*` (X-Admin-Key, 30/min). Dashboards `/admin/referrals.html` + `/admin/costs.html` (Chart.js v4.4.1 + SRI, sessionStorage key cache, `escapeHtml` on inline values).
- **Frontend:** ShareBottomSheet, result-aware Results CTA, ReferralStatusCard, ReferralLandingScreen (deep-link `qaren://` + `https://qaren.app/c/:token`), InviteeQuizScreen, Notifications card (master + 3 sub-toggles), `pushTokenService.tryRegisterPushToken()` (lazy-import, coalesced). Loop 1 honesty: fire `onShared` even if Share intent fails AFTER invite created — never lie about server state.
- **Code redemption is register-only (Bundle A).** `RegisterRequest.invite_code` accepts `^QR-[unambiguous-alphabet]{6}$`; `referral_service.resolve_code_to_invite_id` creates a fresh `referral_invites` row with `source='code_redeem'` then routes through `link_invite_to_user`. No post-hoc redeem endpoint. Deep link `qaren.app/r/{code}` → `qaren://redeem?code=…` → Register pre-filled+locked with × clear affordance. Share copy locked: *"I overthink every purchase. Qaren ends the debate in 30 seconds. Try it: https://qaren.app/r/{code} (or use code {code} in the app)"* — claims decision *closure*, not *correctness*.

### Bundle A Pre-launch P0 (PR #3 merged 2026-05-11 — `f9bf38f`)
Tester-feedback triage shipped via 4-Opus worktree team (`../smartcompare-bundle-a`). Design + plan: `docs/plans/2026-05-11-bundle-a-p0-fixes{,-design}.md`. Session 44 in CONTEXT_SESSION_LOG.md.
- **New screens:** `EditProfileScreen` (name + style + delete; NO change-email — dropped as "useless"), `EditPreferencesFlow` (per-page sequential, NOT scroll — "like Cal AI"), `ContactUsScreen` (POST `/feedback` with bracketed category prefix + `support@qaren.app` fallback), `LegalScreen` (renders `/api/v1/legal/*` markdown via `react-native-markdown-display` with offline cache).
...
- **i18n discipline:** ESLint 9 flat config with `i18next/no-literal-string` rule enforces zero hardcoded user-facing strings. Locale-aware `formatDate`/`formatTimeAgo` swap based on `i18n.language` (`ar-SA` / `en-US`). EN+AR parity 512 = 512.
- **Arabic-as-default: DROPPED** (Session 44 locked decision). Device-locale stays. Do not re-propose default-AR.
...
- **Bundle A deploy status (Session 45, 2026-05-12):** ✅ Backend live. ✅ EAS Update infra wired (`1536aaf`). ✅ Android APK built (`pv5JT6aiXAGkEubbMqZLD2.apk`, runtime 1.0.0, channel `preview`). ✅ Preview channel primed with Bundle A baseline OTA (update group `40719e26`). ☐ Android tester install — pending recruitment this month; Ahmed on iPhone, can't self-QA Android. ☐ iOS build — blocked on Apple Developer ($99/yr, 24-48 hr enrollment). ☐ Production OTA — fires after Android tester walks §7.2.
- **Bundles B/C/D DEFERRED** until Bundle A tester validation (24-72 hr stability). Per `docs/plans/2026-05-11-bundle-a-p0-fixes-design.md` "OUT of scope" table — **B** = Arabic deep clean. **C** = camera 2-product cap (today: 4), type-input "you need TWO products" UX hint, category chip emoji→icon glyphs, in-header logo glyph, Cal-AI micro-animation polish. **D** = bundle size + Reanimated profiling + SVG audit (CohortBarChart 388 dots). **C/D straddler** = Branch.io/AppsFlyer deferred deep-link install survival. **NOT in any bundle:** avatar photo upload (Later — needs S3/Supabase Storage + image picker), Arabic-as-default (DROPPED — do NOT re-propose), phone heat on Expo Go (not a bug — re-eval only if EAS dev build also runs hot), iHerb/Scrape.do (see Known Remaining Bugs), ToS/Privacy content (owned by `docs/plans/2026-05-06-tos-fact-base.md`).
- **GitHub OAuth restore — STILL PENDING after Session 45.** Revoked from KGRddhs during 2026-05-11 incident: VS Code, GitHub CLI, Supabase OAuth. Only Git Credential Manager kept (required for push). Supabase MCP uses service keys, unaffected. Restore = `winget install GitHub.cli` + `gh auth login`, re-authorize Supabase + VS Code OAuth in browser. Revoking an OAuth app does NOT immediately invalidate already-issued tokens — rotate leaked secrets AND revoke the issuing app.

### EAS Update infrastructure (Session 45 — 2026-05-12)
Expo project `@kersher2/qaren` (ID `387a4fcb-76f6-4857-a2fb-39482ca4bd40`). `runtimeVersion.policy: "appVersion"` — bumping `expo.version` forces a rebuild; pure-JS fixes ship via OTA against the existing binary. Channels in `SmartCompareApp/eas.json`: `development` / `preview` / `production`. `appVersionSource: "remote"` (version state in EAS, not app.json).
- **OTA push:** `cd SmartCompareApp && eas update --branch <channel> --message "..."`. Free, no rebuild, lands on next app open. Bundle A baseline = update group `40719e26-b61e-45b0-a1d5-7ee0afbe8c5b`.
- **Rebuild required when:** native module added/removed, app.json plugin/permission changes, `expo.version` bumped. EAS free tier keeps build artifacts ~30 days.
- **iOS build gated on Apple Developer ($99/yr).** EAS handles signing/provisioning once paid; distribute via TestFlight.
...
- **Interactive Expo commands** (`eas login`, build prompts) need a real terminal — Claude's `!` prefix cannot pipe TTY.
