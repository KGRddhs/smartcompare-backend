...
### Bundle history (sessions 44-51)
See `docs/SESSION_BUNDLES.md` — Bundles A (PR #3), B/C/D (PR #4), E (PR #5 + Session 48 merge `e67d583`), Session 49 (D1 scatter-gather + Bucket A 4-bug fix + D2 design+plan), Session 50 (D2 Section 3 + smart-fallback tuning + partial fragrances Tier 1.5 fix, head `378952d`), **Session 51** ((a) D2 follow-up diagnostic: `DEBUG_STAGE_TIMINGS` per-task instrumentation `c38262c`; benched supplements + 3 fragrance pairs cold-cache; documented bottlenecks were stale; resolved as no-op. (b) Bundle C scoring+personalization brainstorm → spec `adb4f2b` → 4-Opus plan `67ae50d`, 5,785 lines / 170 tasks on `feature/bundle-c-scoring`). **Active state:** Bundle E + Session 49 EAS group on `preview` is the latest `eas update`. `STREAM_HARD_CAP_SECONDS=25.0` locks streaming p95 ≤25s. `SCRAPING_MODE=soft` URL gate **wired** at Firecrawl + Scrape.do call sites. Cold-cache wall floor measured Session 51: supplements 11.6s, fragrances 12-16s, electronics 14.3s — all within Session 50 target band (14-17s). Reviews extraction (~5s) + verdict generation (~5s) are the floor-defining stages. **Pending next sessions:** (1) **Bundle C implementation** — 4-Opus team executes 170-task plan; D.1 diagnostic gate BLOCKING for §1a/§1b/§1c bug patches (pros/cons empty, factual_verdict None, prices fall to estimated for mainstream products); (2) **Bucket B brainstorm** dedicated UX session (two-input text/URL boxes redesign). **Worktree-team workflow:** `git worktree add -b feature/<name> ../smartcompare-<name> main` → 4-Opus TeamCreate (`mode: "bypassPermissions"` REQUIRED — sandbox blocks Bash otherwise) → cross-QA → merge `--no-ff`. **Subagent-driven workflow (Session 50):** `Agent(isolation: "worktree")` x2 in parallel for backend-only D2 — smaller than 4-Opus team, validated successful for ~6-8-task scope. **4-Opus planning team (Session 51 pattern):** spawn agents writing to `docs/superpowers/plans/_<bundle>_staging/section_X.md`, then qa-agent assembles into final plan. **Arabic-as-default DROPPED** (Session 44).
