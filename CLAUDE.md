...
### Bundle B/C/D Consolidated (PR #4 ready — Session 46, 2026-05-12)
...
- **Phase 4.5 partial:** perf bench against Railway preview hit pytest --timeout=900 before all 20 cold queries completed; at least one luxury / SPA-scrape query consumed the full 60s per-stream window before emitting `first_paint`. The minimal Task 2.3 impl yields `first_paint` AFTER reviews, which on a cold-cache luxury query means AFTER Firecrawl (~30s) + Scrape.do fallback. Design § Decision 9 wants first_paint ≤13s — **latency tune deferred to Bundle F**. Contract is correct (events exist + ordering invariant holds); the optimisation moves `first_paint` to fire after Serper's initial shopping pull, while Firecrawl/Scrape.do continue async.
- **Deferred follow-ups (post-merge backlog, none blocking):** (a) move `first_paint` yield earlier for P95 ≤14s target, (b) downgrade `rating_service.py:290` US-shopping fallback log from `logger.error` → `logger.warning` (currently fires noisy Sentry on graceful-degradation path), (c) Apple Developer subscription ($99/yr) to ship iOS build for tester manual QA, (d) manual on-device Phase 4.6 (Android emulator when Ahmed home, iPhone after Apple Dev).
