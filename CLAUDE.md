...
### Bundle history (sessions 44-50)
See `docs/SESSION_BUNDLES.md` — Bundles A (PR #3), B/C/D (PR #4), E (PR #5 + Session 48 merge `e67d583`), Session 49 (D1 scatter-gather + Bucket A 4-bug fix + D2 design+plan), **Session 50** (D2 Section 3 shipped — Phase 1 collapse + OpenAI prompt cache + smart-fallback `[:6]`/5s tuning, head commit `4eb1fb7`). **Active state:** Bundle E + Session 49 EAS group on `preview` is the latest `eas update` (no EAS push in Session 50 — backend-only). `STREAM_HARD_CAP_SECONDS=25.0` locks streaming p95 ≤25s. `SCRAPING_MODE=soft` URL gate **wired** at Firecrawl + Scrape.do call sites. **Pending next sessions:** (1) **Supplements price-pipeline optimization** — D2 didn't address iHerb scrape time (~12s wall, drug_context also breaks OpenAI cache prefix uniqueness for supplements); options: parallel iHerb sub-steps, `iherb.com` luxury-style whitelist, or aggressive cache-warming; (2) **Fragrances luxury scrape orchestration** — `fan_out_price_lookup` has no global timeout, ssense.com/Cloudflare scrapes can blow 20+s; needs `asyncio.wait_for` wrapper OR Firecrawl tuning OR accept current behavior; (3) **Bucket C brainstorm** re-scoped from real post-D2 behaviour; (4) **Bucket B brainstorm** dedicated UX session (two-input text/URL boxes redesign). **Worktree-team workflow:** `git worktree add -b feature/<name> ../smartcompare-<name> main` → 4-Opus TeamCreate (`mode: "bypassPermissions"` REQUIRED — sandbox blocks Bash otherwise) → cross-QA → merge `--no-ff`. **Subagent-driven workflow (Session 50):** `Agent(isolation: "worktree")` x2 in parallel for backend-only D2 — smaller than 4-Opus team, validated successful for ~6-8-task scope. **Arabic-as-default DROPPED** (Session 44).
