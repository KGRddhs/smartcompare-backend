...
- **⚠️ CI-GREEN IS NOT ALL-TESTS-PASS — do not quote it as if it were.** `backend-tests` is green because the "Run unit tests" step builds `$DESELECT` from the committed `tests/.pre_impl_failures.txt` and **skips those 14 nodes**. They are still RED on a plain local run: re-verified 2026-09-02, e.g. all three `tests/test_camera_vision.py::TestIdentifyProductsMocked` nodes still fail `TypeError: '>' not supported between instances of 'MagicMock' and 'int'` at `openai_service.py:109` (a fixture defect, not a product bug). The deselect is a REGRESSION gate — it stops NEW breakage — and it is doing that job correctly; it is not evidence the suite passes. Fixing the 14 is **`#89`**, still open. When reporting CI status, say "green with 14 known nodes deselected", never "0 failed".
