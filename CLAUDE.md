...
- **Deferred follow-ups** (post-merge, none blocking): Apple App Store ID swap in Worker (`idTBD` → real), wrangler v3→v4, dead-deps cleanup (`expo-blur` candidate), 5 pre-existing hardcoded English `Alert.alert` strings in HomeScreen/ProfileScreen (origin commit `52ce8957`, ESLint `jsx-text-only` mode doesn't inspect function-call args).
