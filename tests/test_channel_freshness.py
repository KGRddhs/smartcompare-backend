...
# The same page with the newest group a roll-back-to-embedded publish.
ROLLBACK_LIST = copy.deepcopy(FAKE_LIST)
ROLLBACK_LIST["currentPage"][0]["isRollBackToEmbedded"] = True
...
def _rollback_view():
    """`eas update:view <group> --json` for a roll-back-to-embedded group.
    eas-cli 18.8.1 `build/update/roll-back-to-embedded.js`
    (`publishRollbacksAsync`) publishes the group with only `branchId`,
    `rollBackToEmbeddedInfoGroup`, `runtimeVersion`, `message` and
    `awaitingCodeSigningInfo` — NO git commit hash — so the server stores null
    and `sanitizeValue` drops the key. A rollback fixture that carries a hash
    describes a payload eas-cli cannot produce, and hides whether the
    ROLLBACK check runs before the hash-count check."""
    entries = _view(PHONES, drop_hash=True)
    for entry in entries:
        entry["isRollBackToEmbedded"] = True
    return entries
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "[dotenv@16.4.5] injecting env (0) from .env\n", id="bracket-tag-line"
        ),
        pytest.param("{warning} config key ignored\n", id="brace-led-chatter"),
        pytest.param("warning: config {x} ignored\n", id="brace-mid-line-chatter"),
    ],
)
def test_load_json_document_skips_a_chatter_line_that_looks_like_json(preface):
    """A log line that merely STARTS with `[` or `{` (the common `[tag] ...`
    convention) is a candidate that fails to decode; the loader must move on to
    the next candidate, not give up. The ci.yml resolve step imports this same
    function to pick the group, so this is also the CI loader's contract.
    Mutation that reddens this: raising on the first candidate that fails."""
    assert load_json_document(preface + json.dumps(FAKE_LIST)) == FAKE_LIST
    view = _view(PHONES)
    assert load_json_document(preface + json.dumps(view)) == view
    with pytest.raises(ChannelError):
        load_json_document(preface + "[still not json\n")


def test_parse_list_reads_the_rollback_flag():
    """`isRollBackToEmbedded` is the only list field that changes a verdict
    (ROLLBACK, exit 4) on its own; it must be READ from the list JSON, not
    defaulted. Mutation that reddens this: hardcoding
    `is_rollback_to_embedded=False` in parse_list."""
    assert parse_list(ROLLBACK_LIST).is_rollback_to_embedded is True
    assert parse_list(FAKE_LIST).is_rollback_to_embedded is False
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A rollback carries NO hash (see _rollback_view), so this case only
        # passes while the rollback check runs before the hash-count check.
        pytest.param(
            set(),
            ...
            ROLLBACK,
            None,
            id="rollback-to-embedded-has-no-hash",
        ),
        pytest.param(
...
def test_merge_base_error_status_is_a_channel_error_not_not_on_main(
    monkeypatch, tmp_path
):
    """`git merge-base --is-ancestor` exits 0 (ancestor) or 1 (not); any other
    status is an error — an unknown main ref exits 128 (measured on git 2.52:
    `fatal: Not a valid object name nosuchref`). Reading 128 as "not an
    ancestor" blamed the publish (exit 3, ::error::) for a broken invocation;
    it must be a ChannelError, which main() maps to exit 2. Mutation that
    reddens this: `returncode == 0` as the boolean."""
    _record_git(monkeypatch, returncode=128, stdout="")
    with pytest.raises(ChannelError):
        GitAncestry(tmp_path, "nosuchref").is_ancestor_of_main("abc")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            ROLLBACK,
            "::error::",
            id="rollback-parsed-from-list-is-an-error",
        ),
        pytest.param(
            FAKE_LIST,
            _view("sidesha"),
...
def test_main_exit_codes_and_annotations(
    ...
    Whenever both documents parse, an UNANNOTATED context line names the group
    and the view JSON's `createdAt`: the list JSON has no date, so this line is
    how the first armed run confirms `currentPage[0]` is the newest group
    (ruling 3).

    Mutations that redden this: swapping the error/warning prefixes; annotating
    ROLLBACK or NOT_ON_MAIN with `::warning::`; dropping the context line."""
    ...
    if list_payload["currentPage"]:
        assert any(
            "g-newest" in ln and "createdAt=2026-09-01T22:34:56.000Z" in ln
            for ln in plain
        ), f"no context line with the group and createdAt: {plain}"


def test_main_honours_main_ref_and_max_behind(tmp_path, monkeypatch, capsys):
    """`--main` and `--max-behind` reach the verdict. Both are passed here with
    NON-default values, so an implementation that ignores either one and
    falls back to `origin/main` / 1 cannot pass: the phones' hash is 3 behind
    in FakeAncestry, STALE at the default 1 and FRESH at 3. Mutations that
    redden this: main() passing DEFAULT_MAIN_REF instead of `--main`, or
    DEFAULT_MAX_BEHIND instead of `--max-behind`."""
    monkeypatch.delenv("GITHUB_STEP_SUMMARY", raising=False)
    list_path = tmp_path / "preview-list.json"
    view_path = tmp_path / "preview-group.json"
    list_path.write_text(json.dumps(FAKE_LIST), encoding="utf-8")
    view_path.write_text(json.dumps(_view(PHONES)), encoding="utf-8")
    seen = {}
    ...
    argv = ["--list-json", str(list_path), "--view-json", str(view_path)]
    rc = main(
        argv + ["--main", "upstream/trunk", "--max-behind", "3"],
        ancestry_factory=factory,
    )
    assert rc == FRESH
    assert seen["main_ref"] == "upstream/trunk"
    # the same documents at the default distance are STALE — the flag matters
    assert main(argv, ancestry_factory=factory) == STALE
    assert seen["main_ref"] == "origin/main"
    capsys.readouterr()


# ---------------------------------------------------------------------------
# 7b — a fault in the checker is "cannot measure" (2), never "stale" (1)
# ---------------------------------------------------------------------------


def _git_missing(monkeypatch):
...
@pytest.mark.parametrize(
    "list_bytes, break_git, expected_class",
    [
        pytest.param(
            b"\xff\xfe" + json.dumps(FAKE_LIST).encode("utf-8"),
            False,
            "UnicodeDecodeError",
            id="undecodable-list-file",
        ),
        pytest.param(
            json.dumps(FAKE_LIST).encode("utf-8"),
            True,
            "FileNotFoundError",
            id="git-missing-from-path",
        ),
    ],
)
def test_main_maps_any_unexpected_fault_to_unresolvable(
    tmp_path, monkeypatch, capsys, list_bytes, break_git, expected_class
):
    """Only ChannelError used to reach exit 2. Anything else escaped as a
    traceback with Python's exit status 1 — the STALE code — and no
    annotation, so a runner or encoding fault read as "phones are stale"
    (both cases reproduced through the real CLI in review). main() must
    catch it, annotate `::warning::` naming the exception class, and return
    UNRESOLVABLE. Mutation that reddens this: dropping main()'s
    ...
    assert annotated[0].startswith("::warning::"), annotated[0]
    assert expected_class in annotated[0], annotated[0]


def test_unwritable_step_summary_keeps_the_verdict(tmp_path, monkeypatch, capsys):
    """The verdict is printed before the step summary is appended. A summary
    path that cannot be opened must neither replace the verdict with a
    traceback (exit 1) nor change it: STALE stays STALE, plus one
    `::warning::` naming the class. Mutation that reddens this: dropping the
    `except OSError` around the summary write."""
    monkeypatch.setenv(
    ...
    rc = main(
        ["--list-json", str(list_path), "--view-json", str(view_path)],
        ancestry_factory=lambda repo, main_ref: FakeAncestry(),
    )
    lines = [ln for ln in capsys.readouterr().out.splitlines() if ln.strip()]

    assert rc == STALE
    assert any(ln.startswith("::error::") for ln in lines), lines
    warnings = [ln for ln in lines if ln.startswith("::warning::")]
    assert len(warnings) == 1 and "FileNotFoundError" in warnings[0], lines
