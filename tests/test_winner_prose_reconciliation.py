...
* **UNFLAGGED safety repair** (#99, pinned in
  ...
  `verdict` event), the declaration is dropped, and GPT prose that names the
  LOSER is replaced by the qualitative fallback. Shipping the losing product's
  name is indefensible, so it must not sit behind a flag nobody has flipped.
...
(`test_flag_off_repairs_the_name_but_keeps_clean_gpt_reason`,
...
def test_flag_off_keeps_a_gpt_reason_that_does_not_name_the_loser(reconcile_off):
    """Flag OFF is a SAFETY repair, not a rewrite: prose that never names the
    loser is preserved verbatim even on a mismatch."""
