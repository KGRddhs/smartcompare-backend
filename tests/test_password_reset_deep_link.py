...
def test_a_codeless_service_failure_is_400_RECOVERY_FAILED_never_a_500():
    """The generic arm of ``_categorize_auth_error`` returns NO ``code`` (e.g.
    the admin ``update_user_by_id`` rejecting a password under a Supabase
    policy).  The route's ``RECOVERY_FAILED`` fallback is what keeps that a
    structured 400 instead of a ``KeyError`` -> 500."""
    with patch(
        "app.api.auth_routes.complete_password_recovery",
        new=AsyncMock(
            return_value={
    ...
    assert response.status_code == 400, response.text
    body = response.json()
    assert body["code"] == "RECOVERY_FAILED", body
    assert body["success"] is False


def test_an_upstream_blip_keeps_its_retryable_code_at_the_top_level():
    """The screen tells "try again, your link still works" from "your link is
    dead" by this code alone, so it must survive the envelope unchanged."""
    with patch(
        "app.api.auth_routes.complete_password_recovery",
        new=AsyncMock(
            return_value={
...
async def test_complete_password_recovery_verifies_then_writes_then_burns(monkeypatch):
    ...
    manager.attach_mock(revoke, "revoke")
    ...
    assert ordered == ["get_user", "update_user_by_id", "revoke"], (
        "verify upstream -> write -> burn: the password must never be written "
        "before the token is verified, and the token must not be burned before "
        "the write lands (a failed write would otherwise spend the user's link)"
    )
...
def _transport_failures():
    ...
    # The first three are the MEASURED shapes on the pinned supabase-auth
    # 2.31.0: a transport failure escapes `get_user` as the raw httpx error,
    # and a GoTrue 5xx arrives as AuthRetryableError. The first two carry text
    # the W1-4 term list does not match, so only the type check catches them.
    return [
        (httpx.ConnectError("[Errno 11001] getaddrinfo failed"), "DNS failure"),
        (httpx.ReadTimeout("The read operation timed out"), "read timeout"),
        (AuthRetryableError("Server error '502 Bad Gateway'", 502), "GoTrue 5xx"),
        (Exception("Connection reset by peer"), "W1-4 text class"),
    ]


@pytest.mark.parametrize(
    "exc,label", _transport_failures(), ids=[lbl for _e, lbl in _transport_failures()]
)
@pytest.mark.asyncio
async def test_an_upstream_blip_during_get_user_is_retryable_not_a_dead_link(
    monkeypatch, caplog, exc, label
):
    """A blip is not a verdict on the token. Reporting it as
    RECOVERY_TOKEN_INVALID sends the screen to the "link expired" card and
    forces a new email, although the link still works (and GoTrue's verify
    link in the email is single-use). It must come back as the W1-4
    UPSTREAM_UNAVAILABLE shape, with no write and no burn."""
    auth_client = MagicMock()
    auth_client.auth.get_user.side_effect = exc
    admin_client = MagicMock()
    revoke = _patched_service(monkeypatch, auth_client, admin_client)
...
@pytest.mark.asyncio
async def test_a_genuine_upstream_verdict_stays_a_dead_link_whatever_its_text(monkeypatch):
    """The other branch of the split. A GoTrue 4xx is a verdict on the token,
    even when its message happens to contain a transient-looking word; and a
    `UserDoesntExist(token)` whose text IS the bearer must not be classed by
    whatever the base64 happens to spell (the scrub runs BEFORE the match)."""
    from supabase_auth.errors import AuthApiError, UserDoesntExist

    dns_token = "hdr." + _payload_segment(RECOVERY_TOKEN) + ".DnS"
    cases = [
        (RECOVERY_TOKEN, AuthApiError(
            "invalid JWT: unable to parse or verify signature, token is malformed",
            403, "bad_jwt",
        )),
        (RECOVERY_TOKEN, AuthApiError("Session from session_id claim: connection closed", 403, None)),
        (dns_token, UserDoesntExist(dns_token)),
    ]
    for token, exc in cases:
        ...
        assert result["code"] == "RECOVERY_TOKEN_INVALID", (type(exc).__name__, result)
        admin_client.auth.admin.update_user_by_id.assert_not_called()
        revoke.assert_not_called()
...
@pytest.mark.asyncio
async def test_a_user_response_carrying_no_user_is_refused(monkeypatch):
    """``get_user`` returned a response object whose ``.user`` is None -- a
    distinct shape from a bare ``None`` (node 12).  It must be the explicit
    ``RECOVERY_TOKEN_INVALID`` refusal, not an ``AttributeError`` swallowed
    into the generic codeless failure."""
    auth_client = MagicMock()
    auth_client.auth.get_user.return_value = MagicMock(user=None)
    admin_client = MagicMock()
    revoke = _patched_service(monkeypatch, auth_client, admin_client)
...
@pytest.mark.asyncio
async def test_a_failure_after_the_gate_is_scrubbed_and_does_not_burn_the_token(
    monkeypatch, caplog
):
    ...
    admin_client.auth.admin.update_user_by_id.side_effect = Exception(
        "policy rejected for " + RECOVERY_TOKEN
    )
    revoke = _patched_service(monkeypatch, auth_client, admin_client)
