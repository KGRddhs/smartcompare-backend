...
STRIP-CHECK, MEASURED (fix removed, this file kept, ``pytest`` re-run): **7 of 14
fail on base** — ``test_flag_defaults_off``, ``test_flag_is_read_per_call_not_cached``,
``test_on_foreign_card_currency_converts_and_stamps_converted_usd``,
``test_on_unreadable_card_currency_pends``,
``test_on_page_level_currency_used_when_the_card_is_silent``,
``test_on_foreign_signal_with_non_bhd_ask_pends`` and the AST tautology pin
``test_no_variable_is_compared_against_its_own_alias``. Those seven are the
load-bearing set.

The other seven pass on base ON PURPOSE and must not be mistaken for defect pins:
the four ``test_off_*`` cases are FLAG-OFF byte-identity pins (passing on base is
literally what they assert), and ``test_on_matching_card_currency_stays_local_bhd``
+ ``test_on_no_signal_keeps_the_regional_storefront_assumption`` are ARM-PRESERVATION
pins — the fix must NOT start pending an unlabelled page or relabelling a page that
already agrees, which is the acceptance criterion that keeps
``tests/test_iherb_native_bhd_label.py`` green.
