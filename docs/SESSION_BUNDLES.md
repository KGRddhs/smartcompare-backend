...
## Session 62 — Bundle B S3 "Sources" (accuracy + cache lever, SHIPPED 2026-06-14)

**SHIPPED.** Dispatcher session (PoC-driven) extending the S3 worktree-team sourcing into prod accuracy + the price-cache architecture. Main: `5f137ec`(S2) → `6c5c549`(S3 sourcing, prior session) → `43e7711`(accuracy guards) → `e08ddba`(warmer cron).

**Serper depletion + rotation (pivotal op event):** mid-session the live key `05d552d7…` hit "Not enough credits" on EVERY call → prod silently degraded to `estimated` (broke estimate-share=0%). Root: free serper.dev accounts give a FINITE ~2,500 one-time credits; cumulative S2/S3 evals burned through it. Rotated to **`696e4e57…`** (Ahmed created; first misread by dispatcher as a commit SHA — both are 40-hex). Synced LOCAL `.env` + set Railway via `railway variables --set` + git-push redeploy. Cold prod scrape post-rotation = `converted_usd` (Serper confirmed live).

**Price-cache PoC PROVEN:** off-clock warm (`PRICE_RACE_TIMEOUT=60`, no request clock) scrapes genuine → `set_cached` (NOT nocache-gated) → shared prod Upstash → live request (15s clock) serves genuine INSTANTLY. iPhone 244.99 genuine + fragrances 12/16 genuine served from cache on the live clock. Scripts: `.qa-bias-rerun/_cache_warm_poc.py`, `_deploy_verify.py`, `_galaxy_detail.py`.

**Wrong-scrape accuracy guards (`43e7711`, "no wrong scrapes", category-general, 35/35 tests):** the PoC surfaced two genuine-cascade failure modes — (1) an 11.9-BHD Galaxy S24 "case" (accessory the keyword filter missed) → `is_implausible_high_value_price(name, amt)` rejects <50 BHD for is_high_value_query products, wired at fan_out winner + Algolia selector + cited-price veto; (2) a 537-BHD S24-Ultra phantom (GPT-organic snippet, retailer "Bin Hindi", NO url, stamped local_bhd) → wrong-SKU deviation guard drops a cited price >2.5x/<0.4x off the parked `converted_fallback` ref (Galaxy 537 vs 126 = 4.3x → drop → honest converted 126.34). A blunt "require url" first attempt broke 5 cascade tests (the contract allows no-url local_bhd) → reverted for the deviation approach. Verified live across electronics (honest) + fragrances (genuine).

**70%-genuine warmer BUILT + deployed dormant (`e08ddba`):** `scripts/cron_warm_price_cache.py` productionizes the PoC — off-clock genuine scrape of the gold-set catalog → cache. Flag `ENABLE_PRICE_CACHE_WARMER` (fail-closed OFF), `WARMER_SUBSET` (smoke20|full), `MAX_QUERIES_PER_RUN` (25) + Redis rotation cursor `warmer:cursor`, 5/5 unit tests (`tests/test_cron_warm_price_cache.py`). **Engineering blocker (latency wall) SOLVED; the 70% number now = Serper BUDGET:** free Serper is finite + shared with live traffic, so continuous full-catalog warming needs PAID Serper; activation (register Railway cron + flip flag) is an Ahmed decision. Running the warmer locally is classifier-blocked for the dispatcher (prod-cache write) — Ahmed runs the demo via `!`.

**Prod state:** honest + accurate. Serper live; no wrong scrapes (11.9/537 gone); no estimates; cold scrapes → honest converted (latency wall); cached + light-scrape categories (fragrance BH-Shopify, supplement iHerb) → genuine. estimate-share=0% restored.

**Deploy-ops learned:** railway CLI authed (`kinghaleem999@gmail.com`, `empowering-enthusiasm`/`web`/`production`) even when railway MCP is logged out (+ MCP `list_variables` is classifier-blocked as a secret dump). `railway variables --set` quiet on success; `railway` NOT on the python-subprocess PATH; deploy commit at `environments.edges[].node.serviceInstances.edges[].node.latestDeployment.meta.commitHash` (NOT top-level). Railway auto-deploys main on push (~90s); push redeploy applies CLI-set env.

**CARRY-OVER:** (1) the 70% number — activate the warmer cron + paid Serper (Ahmed); (2) supplements are converted_usd not genuine (iHerb USD) — a genuine BH supplement source would lift genuine-share; (3) eval measures COLD (`nocache=true`) — a cache-reading eval variant is needed to measure the warmer's genuine-share; (4) S3 worktree-team disposal; (5) `#42` Jarir SAR→local_bhd saudi phantom; (6) `#37` Algolia variant hardening; (7) full-200 genuine-share eval once the warmer is active.

## Session 63 — Fragrance result-quality + personalization + results-redesign + CATEGORY-FAIRNESS standard (SHIPPED 2026-06-16/17 — main `b7af94d`)

**SHIPPED.** A long dispatcher-led arc (brainstorm → design → diagnosis → ship) that RESOLVED Session 62's on-device Thrust C (6 fragrance content bugs) AND extended into a results-screen 1:1 redesign + a category-wide fairness standard. Solo-dispatcher with SEQUENTIAL subagents (no fleet — weekly budget hit 91%+; sequential because parallel subagents race the git index and the FE work needs the main tree's `node_modules`). Design: `docs/plans/2026-06-16-fragrance-quality-personalization-results-redesign-design.md` + `-plan.md`; handoff: `memory/project_fragrance_quality_personalization_redesign.md`.

**THE KEYSTONE (`115244b`):** the broken fragrance result AND the "is personalization wired?" worry shared ONE root cause — the LLM `category` ("Fragrances", capital F) was never canonicalized, so the lowercase-keyed lookups (scoring dims, spec schema, priority adjustments) silently fell to `"other"`. `canonicalize_category()` (extraction_service; set = `frozenset(CATEGORY_SPEC_SCHEMAS.keys())` so it can't drift) at parse time + defensive guards at the 4 lookup sites fixed C3 ("Build" dim → scent dims), C4 (generic schema → blank 2nd-product specs), AND fragrance personalization (priorities now reweight scent dims) in one move. Ahmed had run it logged-in-with-prefs, so this was a real personalization bug, not expected behavior.

**CONTENT + PERSONALIZATION (`115244b`):** C1 = **price-pending** (`is_price_showable`/`make_pending_price` → estimated/sample/non-genuine → `{amount:None, unavailable:True, reason}`; FE shows "Pricing lands in an upcoming update."; genuine ∪ converted_usd still show — Ahmed's "estimates unacceptable, show genuine or nothing"); C5 bare-`[N]` citation cleanup; C6 `_dim_reviews` honors `rating_derived` (no star-delta vs a displayed N/A). The **cohort proof line was HARD-DEAD** — FE read `result.cohort_summary` the backend never emitted → now emitted `{peer_count, governorate}` with a REAL survey N (`cohort_priors.json` `cohort_match.n`). Priority personalization already worked; the keystone unblocked it for fragrances.

**FE 1:1 REDESIGN (`115244b` Phase 4 + `748a409` accordion; EAS `b5691a77`→`8b7d29c7`):** rebuilt the results screen against the design-handoff `ui_kits/mobile/ResultsScreen.jsx` — split dimension bars + "A · B" legend, dot confidence pills, ★ TOP MATCH, "Why this fits you" + weighted subline, cohort box, value·CENTERED-label·value specs (removed the Show-differences toggle + Highlights block), per-source review quotes, ProsConsCol pros/cons, Accurate/Detailed/Fast chips, price-pending UI. The accordion restructure was a SECOND pass after I wrongly claimed (from screenshots) the FE already matched — the bodies had diverged. **Lesson: verify FE against the actual reference FILE, not a screenshot glance.**

**REVIEWS + SIZE FAIRNESS (`159b4d4`, `a796bdf`):** `build_retailer_quotes_from_reviews` (review_service) emits ≤3 per-source quotes from REAL organic snippets via the `[snippet_N]` map — NO AI-fabricated ratings (rating only when real). Same-size genuine re-selection from retained `self._price_candidates`.

**CATEGORY-FAIRNESS STANDARD (`780d243` config+reconcile, `b332043` size-capture, `b7af94d` target-rules):** Ahmed's directive to generalize the fragrance same-size fairness into a single central per-category `CATEGORY_FAIRNESS` config (price_service): electronics→storage GB, fragrances→ml, supplements→count, grocery/makeup/skincare/haircare→weight/volume; fashion/other→None. `target_pair_value`→plan `{mode: honor_each|target|none, target, per_product}`: (1) BOTH values in the user query→**honor each** (show both, verdict flags the diff, no pend); (2) one mentioned OR one product fixed-size→target it + reconcile the other; (3) neither→common standard = largest value BOTH retained-candidate pools share; (4) tolerance (discrete 5% / continuous 15%). `reconcile_pair_fairness` re-selects genuine prices from retained candidates (no new API calls); fragrance ml is the original case (delegates to the byte-preserved `reconcile_pair_sizes`). Real fragrance size now captured from JSON-LD name/og:title/page-`<title>` (`extract_size_ml_any`, ml + fl-oz snapped to standard bottles) → fairness on TRUE sizes (flagship-100ml last-resort-only). + LIKE-FOR-LIKE rule in `COMPARISON_SYSTEM` prompt. Fragrance scents (family + top/heart/base notes + longevity/sillage/concentration) confirmed captured — Ahmed's "is that mentioned?" = yes.

**LIVE-VERIFIED (fresh prod `nocache`):** "iPhone 15 256GB vs Galaxy S24 128GB" → honors both (no unfair pend); fragrance → scent dims + both genuine prices. The user's "still broken" screenshots were **STALE REDIS CACHE** for a re-run query, not a failed deploy.

**DURABLE GOTCHAS:** (a) verify FE against the design-handoff reference FILE, not a screenshot; (b) stale Redis cache (prices 24h / specs+reviews 7d) masks a deploy on a RE-RUN query → use a fresh/different pair or `?nocache=true`; (c) Bash-tool cwd persists into `SmartCompareApp` after a `cd` (relative `tests/` paths miss; python `open()` cp1252 → `encoding='utf-8'`); (d) design-handoff URL `api.anthropic.com/v1/design/h/<id>` is a >10MB gzipped tar (WebFetch fails) → `curl -o` + `tar -tzf`; (e) `eas update` via PowerShell + absolute `Set-Location`, NOT Ahmed-only-interactive (only `eas login`/`eas build` are); (f) transient subagent API 500s recur — retry (3rd attempt succeeded); (g) Railway MCP + CLI both `invalid_grant` (logged out) — verify deploys via a prod API probe.

**CARRY-OVER / WHAT'S LEFT:** (1) on-device verify the redesign + fairness (relaunch app TWICE for EAS two-launch + run FRESH comparisons, not the cached re-run); (2) residual fragrance size blind spot (size only in a variant-widget/image → flagship-100ml default); (3) DEFERRED: B2 smoke20 `--persist` baseline + A4 cache-reading eval variant; (4) warmer cron activation (`ENABLE_PRICE_CACHE_WARMER`, Ahmed); (5) inherited 70% genuine-share number needs the warmer + paid Serper.

## Session 64 — Faithful Results + bundle-next polish (SHIPPED 2026-06-17 — main `5219cac`)

**SHIPPED via a 4-Opus TeamCreate** (`faithful-results`: Backend/Frontend/Test/QA); dispatcher = **gate-not-do** (oversight + hard-gating, never taking team tasks — Ahmed's explicit mid-session directive, `memory/feedback_dispatcher_gate_not_do.md`). Two waves. Dense prod-state lives in CLAUDE.md's "Active runtime (faithful-results...)" callout + `memory/project_faithful_results_shipped.md`; this is the narrative.

**Wave 1 — main bundle (backend `3d870c8` + cleanup `42d17e4`, EAS preview `71044ff7`):** Phase-0 discovery findings → 4-Opus TDD build with a cross-QA ring (every member QA'd another's work; subpar → sent back — e.g. a copy-policy gap in `review_praise` caught by Frontend, fixed by Backend, re-verified). Shipped: free-tier price cache (7d genuine / 30d negative / cache-first-before-scrape gate / hit-rate metadata), wrong-cheap price guards, `category_profile` ×9 + a generic FE CategoryProfile, verdict fixes (runner-up caption un-suppressed, personalization weaving, empty-tradeoffs fallback, partial-path deterministic verdict), `review_praise` paraphrase (no citations, copy-policy-clean), fairness audit, FE prune-to-design. Gate: QA 9-cat prod smoke + unit gate (⊆ canonical 48) + post-deploy eval (specs 0.9875 / factual 1.0 held, winner 0.45, genuine 3/3). Honest finding: the cold-nocache eval = 0% PASS because prices PEND by design (not fake) — the price win is warmer-gated, not a cold-eval metric.

**Wave 2 — bundle-next polish (backend `5999799`, backend-only / no-EAS):** Ahmed kept the team on (twice countermanded disassembly) for 3 follow-ups. **#14** (tradeoff-card fallback) DROPPED-MOOT — grep-verified ZERO FE `overview.tradeoffs[]` consumer; the `key_tradeoff` caption is the only runner-up surface + already covers all categories. **#15** (2nd-product extraction depth) — `_build_specs_prompt` only guided 4/9 cats; added the rest in the DYNAMIC prompt section (OpenAI cache intact), no-fab pinned both ways; prod makeup 2→6 / skincare 2→3 fields. **#16** (F4.1 longevity scorer reconcile) — bounded ordering swap ONLY when both products have a clear ≥1h hours-signal AND scores disagree; post-deploy eval GREEN: winner **0.40→0.50 (+10pp)**, specs/factual HELD. New smoke20 anchor `54b603e8` (supersedes baseline `7a5fc55b`).

**PROCESS LEARNINGS (durable):** (a) **the eval is POST-deploy** — `eval_runner` is a prod-HTTP harness, so it can't measure un-deployed branch code (caught Test from a meaningless pre-deploy 250-cr run measuring prod-without-the-change); (b) **worktrees don't inherit gitignored `.env`** (sibling dotenv-walk) → a partial-cred unit baseline is contaminated (the initial "59" was wrong → canonical **48** once `.env` present); (c) the **Serper budget counter IS key-scoped**, but prove headroom by RUNTIME evidence (a clean 20-query eval, 0 errors + specs held) not a local-Upstash counter read; (d) cross-QA + Test's verify-against-REAL-data caught real defects AND own-harness bugs (collection-abort via missing `importorskip`; a warnings-section parse false-positive that would've falsely blocked the deploy); (e) verify "complete" via `git show`, not the report.

**PENDING Ahmed (zero Claude tokens):** warmer cron + `ENABLE_PRICE_CACHE_WARMER` (the genuine-price lever — cold prices PEND by design until warmed; #16's full longevity effect also lands post-warmer); on-device walk (`.qa-discovery/AHMED_ONDEVICE_CHECKLIST.md`); 2 reversible judgment calls (CategoryProfile placement / fashion short-labels); optional richer structured tradeoffs CARD (flagged, not built).

**Wave 3 — walk-fix wave (backend `2cb4439`, EAS preview `3efa9d81`, 2026-06-18):** Ahmed's on-device walk surfaced 2 issues. **#20** FE results-organization (`502c694`) — suppress limited-value-data dims, single compact **A·B** bar legend, text-overflow fix (long names/labels were clipping off-page). **#21** BE free image tier (`ea6f287`) — new **Tier 1.5b `extract_image_from_search`** pulls a product image from the already-fetched unified Serper KG/organic payload ($0, strictly-additive); fixes the "image placeholders show no images" complaint. HELD: og:image-fetch fallback (build only if KG/organic proves insufficient). **🔑 SERPER KEY `696e4e57…` DEPLETED (2026-06-18):** raw `serper.dev/search` returns 400 "Not enough credits" → prod DEGRADED (no genuine specs/prices/images on FRESH compares) until rotated. **Rotation = Ahmed's action (PAID recommended** — free serper.dev keys are a finite ~2,500 ONE-TIME allotment, so depletion recurs; paid also unlocks sustained warmer runs). This gates: #21 image live-verify, Ahmed's on-device re-test, and the warmer. **WALK LEARNINGS (durable):** (a) **stale-cache-masks-the-fix** — the walk's "missing specs / wrong dims / no scent profile" screenshots were a 7-day stale specs cache serving a PRE-fix payload; a fresh `nocache` pull confirmed the fix works → always re-test a FRESH pair or `?nocache=true` before believing "still broken" on a fresh deploy (optional un-built fix: a deploy-version cache-bump); (b) **dispatcher headroom mis-judgment** — I dismissed QA's near-cap canary (active key at 2134/2500, which I mislabeled "old") + over-trusted ONE clean 20-query eval as "headroom proven"; cumulative session burn depleted the near-cap key → a clean run is point-in-time proof, NOT sustained headroom; near a cap, heed the counter + track cumulative burn; (c) don't pipe-truncate long-running native commands (`eas update … | Select-Object -First 15` killed the publish mid-flight, exit 255). Team `faithful-results` still on STANDBY (not disassembled) pending Ahmed.

---

## Session 65 — BH source-intelligence SHIPPED + genuine-price DISCOVERY catalog (2026-06-25)

> (Follows the post-#64 ships tracked in `memory/` — catfix `698006a`, fragrance-content-quality `2244ad4`, genuine-bh-price `fb8696c`. This session: ship the BH adapters branch + a 4-round source-discovery sweep.)

**Part 1 — BH source-intelligence adapters SHIPPED (main `e6afbcd`).** Resumed the frozen `feature/bh-source-intelligence` branch (3 $0 genuine-BH adapters: nasserpharmacy `json_api` + bolo/boutiqaat `sitemap`). Applied the Codex re-review #3 round-4 fixes **dispatcher-direct** (not per-fix Workflow waves — the validated anti-pattern): the MEDIUM `_sitemap_cold_domains` served-copy leak `pop` + strengthened the H5/M4 test-discrimination (each **verified fail-without-fix** via disposable revert). A 6-lens whole-branch review **Workflow** (replaced the desktop Codex re-review #4) → CLEAN, 0 merge blockers; it also caught a NEW finding the prior 3 Codex rounds missed — the live nasser token in an untracked `.qa-bh-sourcing/` recon dump → gitignored `.qa-*/`. **SQUASH-merged** to main `e6afbcd` (single-parent; the literal token in ancestor `b75c424` NEVER landed) → pushed → deploys INERT (cron OFF + token gate). **nasser ACTIVATED**: `NASSER_GUEST_TOKEN` re-scraped from the FE bundle + Ahmed set it on Railway + prod-verified (skincare compare → `nasserpharmacy.com` `local_bhd`, CeraVe 13.341 BHD). Handoff `memory/project_bh_source_intelligence_branch_ready.md`.

**Part 2 — genuine-price DISCOVERY CATALOG: 4 ultracode `Workflow`-tool rounds → 400 verified BH/GCC price sources.** Query-driven (not just `.bh`/`.me` — the sephora.me lesson), verify-or-omit. **400 unique sources** (380 $0-scrape, 152 genuine-BHD, 20 render-tier; BH 82 / KSA 64 / UAE 63 / OM 39 / QA 38 / KW 29). **Disproves the prior luxury-fragrance/grocery/supplements "structural gap"** — every category now has genuine-BHD $0 sources. R1 (85, foundation Shopify/Woo) → R2 (214, breadth + platform map) → R3 (83, cracked the giants) → R4 (18, Salla-harvest + last giants; saturated). **The GCC long tail is PLATFORM-STEREOTYPED** (each platform = one canonical $0 extraction); **`curl_cffi impersonate=chrome` defeats most CF/TLS walls** → cracked noon BHD / extra-Unbxd / al-dawaa-OCC / nahdi+danube-Algolia / panda-API / Alshaya-GraphQL family / sporter+drnutrition genuine-BHD supplements (the hardest category — SOLVED) / the Salla storefront API. Catalog COMMITTED on branch `feature/bh-gcc-source-catalog` (`data/bh_gcc_source_candidates{,_round2,_round3,_round4}.json` + 4 `docs/investigations/2026-06-25-bh-gcc-price-source-discovery{,-round2,-round3,-round4}.md`).

**NEXT = the BUILD (fresh ultracode session — faster: clean rate-limit throttle + lean context):** 6 new adapter shapes (salla-api / woo-store-api / occ-rest / alshaya-graphql / algolia / unbxd) + ~400 `Source(...)` rows + `_GENUINE_BH_SOURCE_METHODS` additions (7d TTL) + `verify_source_registry` liveness gate. NO new architecture — reuse the `source_router` registry + 2-layer cache + the 2 off-clock crons; Shopify/Woo/Salla discovery is Serper-FREE = the warmer unblock. 20 render-tier (Akamai: sephora.me/namshi/Carrefour-MAF) deferred. **Build plan: `docs/plans/2026-06-25-bh-gcc-source-build-plan.md`; handoff: `memory/project_bh_gcc_source_discovery.md`.**

**PROCESS LEARNINGS (durable, → CLAUDE.md):** (a) `curl_cffi impersonate=chrome` cracks most CF/TLS walls — "render_required" is often just a TLS wall; probe the platform JSON API directly (usually unwalled even when the HTML PDP is). (b) Wide Workflow fan-out trips a server-side rate-limit burst that WORSENS with session age + RESETS in a fresh session → run heavy fan-out fresh, batch ≤4 late in a long session (an 18-wide launch wiped in 22s; batched-4 survived; web-research rounds are inherently ~1-2.5h). (c) Unescaped backticks inside a backtick-delimited Workflow template-literal prompt break the script parse → use `array.join('\n')`. (d) `GET /prices/{product}` ignores `nocache` + passes no category → can't verify a category-gated adapter; use `compare?q=…&nocache=true`. (e) `.qa-*/` recon dumps can hold live tokens → gitignored; never `git add -A` near them (squash-merge surfaced one).

---

## 2026-06/07 arc (archived from CLAUDE.md 2026-08-30)

> These seven blocks lived in CLAUDE.md as Active-runtime entries until the 2026-08-30 trim.
> They are reproduced VERBATIM. CLAUDE.md keeps one pointer line per block; the flags, the
> durable gotchas and the eval anchor they introduced stayed in CLAUDE.md and are NOT here.

### 2026-06 bundles roll-up
...
### 2026-06-25 -- BH source-intelligence SHIPPED + genuine-price DISCOVERY CATALOG
...
### 2026-06-26 -- BH/GCC genuine-price source build + Zyte luxury render-tier
...
### 2026-06-27 -- key rotation + all-category genuine-price fixes + the CORRECTNESS reframe
...
### 2026-06-28 -- genuine-price CORRECTNESS structural redesign (PR #9)
...
### 2026-07-01 -- electronics cellular cache-key parity (PR #14)
...
### 2026-07-06/07 -- pre-launch price-discovery arc + scraper-fleet audit + WARMER IS THE LEVER
...
## 2026-08-25/26 fragrance data-provenance branch (archived from CLAUDE.md 2026-08-30)

> Branch-state narrative only. The wave MERGED to main `6205a7c` on 2026-08-30, so this is
> history. The findings that are still load-bearing (only-price-is-scraped, the three killed
> assumptions, the Fragrantica/Parfumo prohibition, the scraper roster, the module-reference
> zero-regression gate, the measurement traps, the zero-network corpora, the one-residential-IP
> caveat) were NOT moved - they remain in CLAUDE.md.

Original CLAUDE.md heading, preserved verbatim (fenced so it does not become a heading here):

```text
## Active runtime (fragrance data-provenance + global scrape validation, 2026-08-25/26) — SUPERSEDED by the MERGED block above; branch state below is historical
```

Branch `feature/fragrance-hybrid-capture` off `8adaefb`. ~~NOT MERGED~~ (merged 2026-08-30, see above). 10+ commits, every behaviour
