...
behind `origin/main`; it is non-blocking until 2026-10-07 and inert until the
...
written `(not recorded)`, never guessed. A date the source line does not carry
but its section header does (a merge or ship date, not a publish timestamp) is
marked `(section date)` and the header line is cited in the notes.

| date | channel | group id | gitCommitHash | runtimeVersion | sourcemaps uploaded | published by | notes |
|---|---|---|---|---|---|---|---|
| 2026-09-24 | preview | `561d2cba-f374-40e8-866b-3bfe6c7c9c3b` | `ab9442ae6625267cd114a5593661ee9b79f1a78f` | 1.0.0 | no — the publishing session had no Sentry token; upload still owed | Fable orchestrator, session 67 (the Expo account logged in on the dev box) | Session 67 set: the W3 client halves (W3-4, W3-6, W3-9, W3-11bcd, W3-14, W3-15, W3-16), R-CLIENT W1-4d logout body, B1 lucide per-icon split, M21/M23 mobile waves — PRs #163–#195. Published 12:06:46 UTC with `--clear-cache`; iOS update `01a0d34f-a2f6-7649-93bf-0a46700434c8`, Android `01a0d34f-a2f6-7578-a9af-7aa09210b108`; the served manifest was verified through u.expo.dev on both platforms (same ids, same group). The first attempt failed at bundling because the clone's `node_modules` did not match the lock (`intl-pluralrules` missing, axios 1.16.1 vs 1.20.0 — CI's `npm ci` hides that class); `npm install --no-save` reconciled it. Owed: the sourcemap upload (`expo-upload-sourcemaps dist` with a Sentry token) and the on-device Arabic walkthrough. Supersedes the 2026-09-02 row below as the live bundle. |
...
| 2026-06-02 (section date) | preview | `18af8a48-a191-4b5d-bc62-9508ab4b5952` | (not recorded) | (not recorded) | (not recorded) | (not recorded) | Bundle E S3 hot-fix wave 1; `docs/SESSION_BUNDLES.md:523`; date from the `:502` header (SHIPPED 2026-06-02). |
| (not recorded) | preview | `1856c8fb-70ea-4333-b402-b09ad7f2af5f` | (not recorded) | (not recorded) | (not recorded) | (not recorded) | Bucket A bugs 1 + 2; `docs/SESSION_BUNDLES.md:89`. |
| 2026-05-13 (section date) | preview | `d540c1e6-c07c-46d7-ac69-5103dde1fb56` | `0129106` | 1.0.0 | (not recorded) | (not recorded) | Bundle E results quality overhaul; `docs/SESSION_BUNDLES.md:36` (commit) and `:47` (full group id, both platforms, runtime 1.0.0); date from the `:34` header (PR #5 merged 2026-05-13). |
