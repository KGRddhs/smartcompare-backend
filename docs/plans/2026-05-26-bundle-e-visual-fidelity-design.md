...
|---|---|---|---|
| **PhoneMockup** | Step03ValueProp (no JSX ref for s3 — frontend lane's spec) | Stylized iOS-shape SVG outline w/ Qaren wordmark inside, emerald accent dot bottom-right. ~180–280px. Phone frame at 3/4 angle (skewed via SVG transform), 2 inner product cards (winner has emerald outline + badge + pulsing glow ring). | Subtle 0.95→1.0 scale-in on mount (320ms cubic-bezier); glow ring pulses 1.4s loop; idle otherwise. |
...
**Dropped:** `CohortBarChart` (Bundle D heritage, no JSX consumer in current spec). If `hero/CohortBarChart.tsx` exists post-`git mv illustrations/ hero/`, frontend should DELETE it (or move to `admin/` if Ahmed wants it for an internal dashboard later — out of scope for Bundle E).
