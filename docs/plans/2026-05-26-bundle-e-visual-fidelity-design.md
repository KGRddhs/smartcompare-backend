...
| `OnboardingWelcomeScreen.jsx` (s1) | `screens/onboarding/Step01Welcome.tsx` | M | Warm-wash radial-gradient bg (orange + blue corner tints); QarenLogo 40px + "Look closer. Decide smarter." headline + "Built for the GCC. By people like you." subtitle; **QuoteRow trio** (3 glass-blur testimonial cards w/ emerald dot + quote text — primitive added in S0.3); Continue CTA + "Already have an account? Sign in" link. **No hero SVG on Step01** (JSX-wins ruling, 2026-05-26). |
...
| `OnboardingCohortScreen.jsx` (s12) | `screens/onboarding/Step12CohortProof.tsx` | M | **CohortBarChart hero SVG**; "73% of Capital shoppers your age prioritize Quality" framing. |
| `OnboardingReadyScreen.jsx` (s15) | `screens/onboarding/Step15Reveal.tsx` | M | **RevealBurst hero SVG** + emerald confetti; "Your advisor is ready" headline. |
...
### 3.2 Hero illustrations (5 components — SVG + Reanimated, ZERO Lottie)

Live under `SmartCompareApp/src/components/hero/`. Each owns its own Reanimated worklet — no shared global animation state.

| Component | Used in | Anatomy (from `.jsx` references) | Motion |
|---|---|---|---|
| **PhoneMockup** | Step03ValueProp | Stylized iOS-shape SVG outline w/ Qaren wordmark inside, emerald accent dot bottom-right. ~180–280px. Phone frame at 3/4 angle (skewed via SVG transform), 2 inner product cards (winner has emerald outline + badge + pulsing glow ring). | Subtle 0.95→1.0 scale-in on mount (320ms cubic-bezier); glow ring pulses 1.4s loop; idle otherwise. |
| **ConcentricMotif** | Step03 (value prop), Step05 (trust), LoadingScreen ConcentricVariant | 3 emerald rings expanding outward from Q logo center, staggered 700ms, looping 2.1s. | `qarenRing` keyframe: `scale(0.8)` opacity `0.9` → `scale(2.5)` opacity `0`. |
| **CohortBarChart** | Step12CohortProof | 8 vertical bars (~24px wide, varying heights 30–90px), emerald fill on the "your group" bar, gray on others. Caption: "Capital · 25–34". | Bars grow from 0 to full height on mount (staggered 80ms each, 480ms total, cubic-bezier(0.32,0.72,0,1)). |
