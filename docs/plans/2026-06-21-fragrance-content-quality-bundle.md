...
### P9 — claude.ai/design for the per-category result contract — RECOMMENDATION: a LIGHTWEIGHT contract pass now, full shell rebuild LATER
**Ahmed's question: use claude.ai/design now to wire the backend result structure per category, or not needed yet?** Recommendation:
- **DO now (cheap, high-leverage): a per-category CONTRACT pass, NOT a full redesign.** Define, per category, the data contract the result MUST provide + how each field renders honestly — for fragrances: scent profile, notes, concentration, longevity/projection EVIDENCE (not a bare number), CONFIRMED-vs-pending price status, REAL review sentiment. This is the artifact P1–P7 SERVE (P3 qualitative summary, P4 pending-price honesty, P6 real review sentiment, P7 spec-row rules all need a target contract to render). Defining it first is literally Ahmed's "render the contract, don't rescue after the fact" principle — the contract has to EXIST before the shell can render it. Output = a documented per-category contract (fields, confirmed/pending/absent states, qualitative-not-numeric rules) + a fragrance reference mock.
- **DEFER: a full shared-shell rebuild.** The category-driven Results shell ALREADY exists (faithful-results 1:1 redesign shipped; `ui_kits/mobile/ResultsScreen.jsx` + `design-sync` infra). A ground-up rebuild is NOT needed now — once the contract (above) + the P1–P7 honesty fixes land, re-sync the EXISTING shell to the cleaned contract via the normal `/design-sync` diff-and-integrate flow, don't rebuild.
- **Why not skip design entirely:** because the atrocious result was partly a CONTRACT failure (the UI rendered a raw score, a pended price next to a soft price-ref, one-sided spec rows) — fixing those in backend alone, without a contract the FE renders to, risks the leak reappearing at a new surface. The contract is the durable fix; the backend P-items enforce it.
