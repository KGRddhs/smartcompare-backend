...
**[BUNDLE B.0 ARCHITECTURAL TASK — SHIP FIRST]:**
- **`source_router` cascade wiring (B0-UnfinishedBiz audit, 2026-06-09):** Lane 2 of Sprint A shipped `app/services/source_router.py` with the Bahrain-first ×3.0 / GCC ×1.5 / Global ×1.0 registry promised in § 4 above — BUT zero production code paths read `SOURCE_REGISTRY`. `get_sources_for_category()` + `score_source()` are defined but never called. The Tier 1.5 escalation cascade in `structured_comparison_service.py` + `price_service.py` still uses the legacy hard-coded `OFFICIAL_BRAND_DOMAINS` (Apple/Samsung/Sephora/Harrods — no `.bh` domains) + `GCC_LUXURY_RETAILERS` (fashion/luxury only). Result: Patchi, Ahmad Tea, Carrier AC, and every non-luxury non-electronics GCC-local brand falls through to Tier 3 GPT-estimate (`metadata.source_trace.price.values[].src == "gpt_training"`) — 0% Bahrain retailer hit rate confirmed by B0-D's 24-query bias matrix. This is the Sprint A unfulfilled promise; every subsequent Bundle B social-source feature (Reddit/YouTube/Instagram per § 6.1) depends on the source-routing layer working. **Estimated effort:** 2-3 days, cross-cuts `price_service.py` + `structured_comparison_service.py` + `serper_service.py` (add `site:<bahrain_domain>` per-source query for non-luxury weak-Tier-1).
...
- **STREAM_HARD_CAP_SECONDS raise 25→30 (Bundle B0-C hardening, 2026-06-09):** Set on Railway by Bundle B0-C hardening per Smoke + Concurrency audit findings — cold-cache canonical pairs in supplements_a (NOW Foods + Solgar D3), haircare_a (Olaplex + K18), and fragrances_a (Tom Ford + Creed) breached the 25s cap. Successful Tier 1.5 + reviews 4-5s + verdict 4-5s stack tight against 25s; the outermost `asyncio.wait_for` in `text_routes.py` `_run_stream_with_cap()` raises `asyncio.TimeoutError` which terminates the SSE prematurely. Raising to 30s preserves the streaming UX contract for these three categories without changing any other behavior. Wall-budget watch still applies — B.6 eval pipeline should track p95 stream-completion latency to confirm the new cap holds.
