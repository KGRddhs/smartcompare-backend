...
> **CORRECTION (2026-07-02, genuine-price KPI recon):** the "SAR regardless of locale headers" claim above is **FALSIFIED** — the `x-locale: en-bh` **HEADER** (not a cookie; `nloc=en-bh` does NOT work) flips `GET /_svc/catalog/api/v3/search?q=…` to the **Bahrain/BHD catalog** over plain `curl_cffi impersonate=chrome` (live-verified: iPhone 15 256GB 447.29 / sale 294.23 BHD vs 3799/2499 SAR without the header). GOTCHA: without the header the response is the KSA/SAR catalog with an IDENTICAL shape and **NO currency field** — the silent wrong-currency failure mode; pin the header on every request. Full request/response contract (paths, `sale_price` fallback, buy-box `offers[0]` authority rule, failure modes): the 2026-07-02 recon `noon_contract` (recon_electronics) referenced from `docs/plans/2026-07-01-warmer-source-quality-blocker.md` fix-ladder item 4 (noon-BH adapter).
