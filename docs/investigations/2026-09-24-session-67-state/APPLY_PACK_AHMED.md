...
## A. Migrations — Supabase SQL editor, in this order (one script per run; the editor wraps each run in ONE transaction, so a "no error" screen is not evidence — the AFTER queries are)
