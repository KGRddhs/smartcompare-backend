## W4-2 `ENABLE_SHOPPING_DISCOVERY_URL_SPLIT` (PO-RECORDED-MEASURED-03) - default OFF, coupled to W4-1

TDD per the Synack Build Orchestrator on Opus 5.5 with Fable gating: red (16 right-reason reds / 35 pins, gated) -> green -> adversary r0 SOUND (5 minors; the private `_discovery_url` reached the wire on `GET /api/v1/text/prices`, fixed) -> fix -> adversary r1 SOUND (3 minors) -> polish (the strip-only pin `test_19c`; PR-text corrections) -> verifier SOUND. Base `6ab9d7ea` (W4-1 merged); rebased onto current main at commit time with the unit file re-run in every flag state.

**THE FLAG STAYS DARK** until Ahmed's product call with the '(converted from USD)' rendering in front of him (W4-1 ruling R5); activation order for the shopping tier: W4-1 -> W4-2 -> W4-3; `ENABLE_REGION_CURRENCY_GUARD` last.
...
- **Mutations:** 27 mutations in the fix round, plus the M0 control run: 16 M-rows (the spec table), 8 N-rows (adversary N1/N1b/N2/N3/N3b/N4/N4b/N6) and 3 D-rows (the regional strip D1a/b/c). Each one turns its named pin red.
  - The r1 adversary re-ran 22 of the 27 with matching red counts and added 15 of its own mutations. 14 went red; X15 (strip every `_`-prefixed key) survived.
