...
- CLAUDE.md corrections still owed (from the retro reds; details in `state/journal_retro-w1-red.json`): the `ENABLE_DEFAULT_RATE_LIMITS` row's "21 routes" figure does not hold on the pinned slowapi/starlette; the W1-4 flag row must say `ENABLE_LOGOUT_UPSTREAM_REVOCATION` is NOT inert until the OTA once the backend calls `admin.sign_out(access_token, "local")` (R-AUTH); the migration apply order becomes: 037 any time (it no longer depends on 035/036), 036 when its flag is readied (re-run the idempotent 037 after a late 036), 040 any time, 038 = W3-16, 039 reserved for the M13-29 RLS migration (R-MIG; the earlier `035 then 036 then 037` prerequisite is withdrawn); the W0-4 row's iHerb sentence, activation-order pairing and loop-gap residual (R-W04, still unmeasured).
