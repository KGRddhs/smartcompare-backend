...
- **Mutation matrix:** from byte snapshots with sha256-verified restores, each row run in both flag states.
  - **W0-4e2 rows.** Every row is red in both states.
    - **This round:** the ten unmarked rows were re-run on the final test bytes against the efg file. The four rows marked ‡ also ran against all six unit files, with the same red nodes: X1 '9 failed, 311 passed', step '20 failed, 300 passed', OVERLAP=100 '3 failed, 317 passed', CHUNK=2M '1 failed, 319 passed', identical in both states.
