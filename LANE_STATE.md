...
- **Capture RUNNING** (~40/200 last signal) → `.qa-i2-refute/full200_bodies.jsonl`. Run: `python .qa-i2-refute/offline_param_sweep.py --bodies .qa-i2-refute/full200_bodies.jsonl`. Awaiting (1) full-capture done signal, (2) team-lead decision on optional Pass C (replace gl=us AMOUNT with gold-band midpoint to measure the price-REPLACEMENT upside — gated to never touch the winner target). Then: sweep → ship Pass-A winning combo as v2 defaults (Ahmed sign-off); B/C are diagnostics not ship.
