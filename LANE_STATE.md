...
- [ ] #21.2 alhajis VERIFY — confirm `_fetch_shopify_currency("alhajisbahrain.com")`->"BHD" + genuine BHD on a real designer fragrance. (awaiting team-lead on the +1 live query; FLAG if not BHD per M1)
- [ ] #21.3 boutiqaat + sephora render-tier Sources (diff to L1).
- [ ] #21.WIRING — hand L1 the EXACT diff: is_algolia flag on Source + 6thStreet registry rows + Tier-2 cascade call-site (between Shopify Tier1 and curl Tier3). L1 #32 is waiting on it.
