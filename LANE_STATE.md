...
**ALL L3 TASKS COMPLETE (L3.1–L3.4) + ESTIMATE-DEMOTION + MECHANISM INVESTIGATION + A2 (value-axis neutralization, DEFAULT ON).** A1 DEFERRED per ruling. GPT-qualitative-winner lever HELD for Ahmed. Awaiting smoke20 measurement-mechanics decision (A2 not yet on prod → needs local-uvicorn run OR fold into gate).

## OPTION A → final disposition (team-lead ruling 2026-06-13)
**A2 value-axis neutralization — KEPT, DEFAULT ON (commit 214e689 + default-flip in 8b5a638).** On cross-tier, recompute winner with value-type dim removed → the cheaper-wins anti-signal (gold winner is pricier 64% across 166 rows) stops deciding the pick. Within-tier untouched. Winner-only (overall/breakdown unmutated). OFF-switch `DISABLE_WINNER_VALUE_NEUTRALIZATION`. The ONE corpus-justified winner move. Tests: tests/test_winner_value_neutralization.py (5) GREEN.
**A1 normalization dampening — DEFERRED + REVERTED (commit b2e4315).** Ruling: calibration/honesty only, won't move winner_pass, carries a dim-bar UX change for zero winner benefit → benched for a future calibration pass. _normalize_dimension restored to legacy 30+ratio*70 + 70 tie; A1 constants + escape hatch removed; 5 dim-literal tests restored to legacy; A1 test deleted. **500 scoring regression GREEN.**
**GPT-qualitative-winner lever (intervention #2) — HELD for Ahmed.** team-lead identified it as the likely-only path to 0.60 (deterministic spec-scorer has a genuine CAPABILITY CEILING — can't see camera-quality/ecosystem/heritage/justified-premium, which is what gold rewards). Real architecture change (decouple forced-deterministic winner + let GPT's qualitative pick win on divergence). team-lead TAKING IT TO AHMED. I do NOT prototype it.
**SAFETY:** A2 winner-only — winner_pass/price/specs/factual eval axes can't regress (winner reads overall; eval price/specs read overview.products).
**A2 MEASUREMENT BLOCKER:** smoke20 hits PROD Railway URL by default; A2 is on my worktree branch, NOT deployed. Pre-merge measurement (condition 3) requires either a LOCAL uvicorn (TARGET_BASE_URL=localhost) serving the worktree + .env creds copied in, OR folding A2 into L4's gate smoke20. Question surfaced to team-lead; awaiting mechanics + CREDIT decision.
