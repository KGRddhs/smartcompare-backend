...
**ALL L3 TASKS COMPLETE (L3.1–L3.4) + ESTIMATE-DEMOTION + MECHANISM INVESTIGATION + OPTION A (A1+A2 DEFAULT, team-lead approved).** Awaiting smoke20 CREDIT GO (condition 3: no L3 merge until A measured) + ruling on intervention #2.

## OPTION A — A1+A2 (commit 8b5a638, team-lead APPROVED as DEFAULT 2026-06-13)
**A1 normalization dampening (DEFAULT ON):** `_normalize_dimension` spread 30+ratio*70 (30–100) → 45+ratio*40 (45–85); genuine-tie 70→65. Stops tiny raw-spec edge manufacturing 70pt landslide. FULL version (compresses BOTH displayed dim bars AND winner/overall — team-lead "more coherent" steer). Escape hatch `DISABLE_DIM_NORM_DAMPENING` (default OFF) reverts to legacy. MISSING propagation untouched. Constants `_DIM_NORM_{FLOOR,SPAN,TIE}_{DAMPENED,LEGACY}`. 5 deliberate dim-literal test updates (30/70/100→45/65/85), invariants unchanged.
**A2 value-axis neutralization (FLIPPED to DEFAULT ON):** on cross-tier, recompute winner with value-type dim removed → cheaper-wins anti-signal stops deciding. Within-tier untouched. Winner-only. OFF-switch `DISABLE_WINNER_VALUE_NEUTRALIZATION`. (Was flag-gated default-OFF in 214e689; ruling flipped it to default.)
**SAFETY:** winner_pass/price/specs/factual eval axes can't regress (winner reads overall; eval price/specs read overview.products). **620 scoring regression GREEN.**
**HONEST MEASUREMENT (4 lane1 mapped fixtures, illustrative):** A1+A2 moves 1/4 to gold (fashion), was 0/4. Other 3 are same-tier spec-numerology cases where the deeper capability gap persists → Option A is a BOUNDED partial fix; intervention #2 (qualitative-verdict decoupling) still needed for the rest. NOT claiming headline lift — smoke20 pending CREDIT GO.
**The HEADLINE lever remains intervention #2** (let the GPT qualitative verdict win on divergence) — AWAITING team-lead ruling (architecture call, inverts H1).
- Tests: tests/test_normalization_dampening_a1.py (8) + tests/test_winner_value_neutralization.py (5) GREEN.
