...
## ✅ OFFLINE SWEEP HARNESS REBUILT + SELF-TEST GREEN — standing by for capture
`.qa-i2-refute/offline_param_sweep.py` (scratch, never committed). Reconstructs compute_scores inputs from each captured body (specs/price.source_method/rating/review_count + `_debug_capture.fact_check`) → per combo re-runs compute_scores → `_build_scoring_v2` → reads `overall_score.winner_idx` EXACTLY as `eval_runner.extract_winner_index`. price/specs/factual held FIXED from capture (guard valid by construction). Grid = WINNER_PRICE_AUTHORITY_POINTS[4/2/6] x WINNER_DIM_GAP_TOLERANCE[.08/.05/.12] x WINNER_VALUE_WEIGHT_SCALE[1.0/.85/.70] x DISABLE_DIM_NORM_DAMPENING[on/off] = 54 combos; guard price>=.84/specs>=.87/factual>=.94/weighted>=.425. **Self-test GREEN (plain Windows console): reconstruct->rescore parity = IDENTICAL winner_idx to direct call; price_authority lever moves winner 0->100%; guard fires.** Run: `python .qa-i2-refute/offline_param_sweep.py --bodies full200_bodies.jsonl`. Awaiting team-lead capture format confirm + the bodies file.

**NEXT (team-lead owns): v2→main ff (4th) → L1 (5th) → deploy → EVAL_CAPTURE_DEBUG + ONE full-200 capture → hand me full200_bodies.jsonl → I sweep → set hatched defaults to winning combo (Ahmed sign-off).**
