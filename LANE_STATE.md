...
- **L1 HEAD bumped `5a4f614` → `365a7ca` (Fix 3 — bounded BH-curl budget).** Verified ORTHOGONAL to my contract via diff (no re-run needed): source_router.py = 0 changes (is_render_only field + helper byte-identical); ssc.py = +24/-1 isolated to a new `_PARKED_FALLBACK_FAN_OUT_BUDGET` (9s) fed to the fan_out `wait_for` WHEN a parked converted exists — does NOT touch `_build_escalation_scrapers` (Fix-B render gate) or the timeout-fallthrough (A5 path). Fix 3 actually REINFORCES A5 (falls back to parked faster). My 16/16 @5a4f614 carries to 365a7ca. Branch HEAD to track = **365a7ca**.
