...
- **FIX (single point):** replaced the `asyncio.run` with a dedicated `new_event_loop()` + `run_until_complete` + `set_event_loop(new_event_loop())` in `finally` — restores a fresh usable default loop so downstream `get_event_loop()` consumers aren't poisoned.
- **VERIFIED:** polluter + ALL 4 victim files (share_routes + http_400_cap_cut_mapping + pharmacy_jsonld + referral_must_fixes) → **145 passed**, only the pre-existing GOOGLE-DIAG console.log remains (the known 103/104 baseline, not loop-related). One `asyncio.run` swap fixed all 12 cross-test failures. Full free-unit suite re-run in progress to confirm suite-wide + no new failures.
- 2 phase1 timing tests (test_phase1_includes_reviews, test_all_fast_races) = load-sensitive false-negatives on slow box (6-7s vs <2s walls) — NOT loop-related, left as known-slow-box flaky.
...
- **L1 HEAD churn `5a4f614` → `365a7ca` (Fix 3) → `cd8257c` (Fix 3 REVERTED, prod-verify-first).** Contract stable since `60e932b` throughout; only orthogonal prod-hardening moved on top. Verified via diff each time (no re-run): `365a7ca` Fix 3 = isolated `_PARKED_FALLBACK_FAN_OUT_BUDGET` (9s) budget value, didn't touch my gate/fallthrough (and REINFORCED A5). `cd8257c` reverts Fix 3 → `git diff 5a4f614 cd8257c` for source_router.py + ssc.py is **EMPTY** (byte-identical to 5a4f614). My 16/16 @5a4f614 carries to **cd8257c** exactly. Branch HEAD to track for the flip = **cd8257c**.
