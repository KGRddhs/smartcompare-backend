...
- Modes: `--selftest` (synthetic), `--realrow <file>` (validate loader on real rows), `--bodies <file>` (the sweep).
- **Capture is RUNNING** (team-lead, ~60-90min) → bodies land at `.qa-i2-refute/full200_bodies.jsonl`. Run: `python .qa-i2-refute/offline_param_sweep.py --bodies .qa-i2-refute/full200_bodies.jsonl`. Awaiting full-capture done signal → sweep → set hatched defaults to winning combo (Ahmed sign-off).
