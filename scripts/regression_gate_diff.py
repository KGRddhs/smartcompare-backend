...
class DiffResult:
    """The three partitions of current-vs-baseline failure sets."""
...
def diff_failures(current: Iterable[str], baseline: Iterable[str]) -> DiffResult:
    """Partition current vs baseline failure-id sets. `new_failures` is the
    regression signal (current minus baseline)."""
    cur = set(current)
    base = set(baseline)
    return DiffResult(
