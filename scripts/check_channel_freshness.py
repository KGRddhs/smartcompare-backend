...
decodes from the first line that starts with ``{`` or ``[`` AND from which the
rest of the text decodes. The ci.yml resolve step imports that same function
(with ``parse_list``) to pick the group it passes to ``update:view``, so the
group the job fetches and the group this script judges are chosen by one rule.
...
``tests/test_channel_freshness.py::test_exit_codes_are_the_documented_contract``:
    ...
    2  UNRESOLVABLE  empty page / no hash / more than one hash / hash unknown here /
                     a git call that failed (e.g. an unknown ``--main`` ref) /
                     any other fault in the checker itself (an undecodable file,
                     git missing) — annotated with the exception class
...
def load_json_document(text: str) -> object:
    ...
    Every line whose stripped text starts with ``{`` or ``[`` is a candidate,
    tried in order; the first one from which the REST of the text decodes wins.
    A ``[tag] ...`` log line (``[dotenv@16.4.5] injecting env ...``) is
    therefore skipped instead of ending the search. This is also the loader
    the ci.yml resolve step imports to pick the group, so the two cannot
    disagree."""
    ...
    if last_error is not None:
        raise ChannelError(f"eas-cli output is not valid JSON: {last_error}")
...
class GitAncestry:
    ...
    def is_ancestor_of_main(self, sha: str) -> bool:
        # git documents --is-ancestor as "0 if true, 1 if not; errors are
        # signaled by a non-zero status that is not 1" — an unknown main ref
        # exits 128 (measured on git 2.52). That is a broken invocation, not a
        # publish from a side branch, so it must read UNRESOLVABLE (exit 2), not
        # NOT_ON_MAIN (exit 3).
        proc = self._git("merge-base", "--is-ancestor", sha, self.main_ref)
        if proc.returncode in (0, 1):
            return proc.returncode == 0
        raise ChannelError(
            f"git merge-base --is-ancestor {sha} {self.main_ref} failed "
...
def evaluate(
    ...
    # ROLLBACK is decided from the list JSON alone and MUST come before any
    # look at ``hashes``: eas-cli 18.8.1 publishes a roll-back-to-embedded
    # group with no git commit hash (build/update/roll-back-to-embedded.js
    # publishRollbacksAsync sends branchId, rollBackToEmbeddedInfoGroup,
    # runtimeVersion, message, awaitingCodeSigningInfo), so its view JSON has
    # no ``gitCommitHash`` key and ``hashes`` is empty. Gated on the hash
    # count, a rollback would read UNRESOLVABLE (exit 2, ::warning::) instead
    # of ROLLBACK (exit 4, ::error::).
...
def main(
    ...
    *,
    ...
    try:
        ...
        # Anything else (a UnicodeDecodeError reading a file, git missing from
        # PATH, a --repo that does not exist) would otherwise escape as a
        # traceback with Python's exit status 1 — which is the STALE code, with
        # no annotation. A fault in the checker is "cannot measure", never
        # "phones are stale": exit 2 with a ::warning:: naming the class.
        code = UNRESOLVABLE
        message = f"channel check failed ({type(exc).__name__}): {exc}"
    if context:
        print(context)
    ...
    if summary:
        try:
            ...
            # The verdict above is already printed; an unwritable summary must
            # not replace it with a traceback (exit 1 = STALE) or change it.
            print(
                f"::warning::cannot append to GITHUB_STEP_SUMMARY "
