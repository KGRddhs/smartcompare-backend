...
## Session 62: M5/M6/M7 fragrance capture waves + docs closeout — 2026-08-30

**Shipped:** M0/M5/M6 merged to `main ddead8d` (Firecrawl `rawHtml` repair, RSC-flight + Magento-GQL/jomashop-PQ + Salla-slug price adapters, Yotpo + judge.me review adapters, normalize_words/sitemap-matcher/PDP-marker/glass-exemption discovery fixes). Branch `feature/m7-specs` (unmerged, unpushed) adds D1 `a6901b7` (closes the smart-fallback/Tier-2/Tier-3 spec-refill cascade under `ENABLE_SPECS_NO_FABRICATION` — the guard alone was leaky), D2 `3f4fd12` (fragrance spec spine, dark, store empty + migration `035` unapplied), D3 `dc64c69` (per-host search descriptor, dark, store empty), D4 = this docs commit. Gate evidence: per-file pytest 159 passed (M5+M6) + 66 passed (M7 D1-D3), 0 failed.

**Durable lessons:** (a) A prompt-level guard is not a guarantee while any downstream tier can refill the same field — D1 existed only because U0.3's guard left three ungated LLM refill sites that read an omitted field identically to `"N/A"`; when adding a "don't invent" rule, grep for every consumer of the falsy value. (b) `default OFF` and `INERT` are different states: D2/D3 ship with EMPTY stores on purpose, so flipping the flag alone changes nothing — activation is *seed-then-flip* (`seed_spec_spine.py`, `resolve_search_descriptors.py`), and the runbook has to say so or a canary will read as "the feature does nothing". (c) `ENABLE_SITEMAP_PDP_MARKERS_V2` only changes what the OFF-CLOCK builder indexes — an existing index does not gain PDPs retroactively, so the flip must be followed by a rebuild before `ENABLE_SITEMAP_MATCH_V2` means anything. (d) `ENABLE_NORMALIZE_WORDS_EMPTY_FIX` is the one wave flag with GLOBAL matcher blast radius (Part B recovers the measured sitemap hits without it), so it gets its own canary window; everything else can be batched by family.

**Activation order recorded in CLAUDE.md** (extractor fixes + price adapters → review adapters → sitemap trio with `ENABLE_SITEMAP_INDEX` → glass exemption → normalize_words alone → seeded M7 stores → `ENABLE_SPECS_NO_FABRICATION` LAST, only after search restoration). Research artifacts stay in the session scratchpad (`research/B5` spec-spine, `research/B8` discovery), not the repo. **CLAUDE.md is oversized (~600 lines) — trim the 2026-06/07 arc + the SUPERSEDED 2026-08-25/26 block into `docs/` next session.**
